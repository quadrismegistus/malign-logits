...
OUT = os.path.join(CAMPAIGN, "results", "r_decoy_selection_stats.parquet")
...
def main():
    ...
    for r in src.itertuples():
        ...
        rows.append(dict(stem=r.stem, member=r.member, prompt=r.prompt, faller=r.faller,
                         pool=len(d), win=counts[0],
                         runner=counts[1] if len(counts) > 1 else 0,
                         margin=counts[0] - (counts[1] if len(counts) > 1 else 0),
                         share=counts[0] / tot if tot else float("nan")))
    df = pd.DataFrame(rows)
    print()
    print("rows: %d of %d  (marked %d, unmarked %d)"
          % (len(df), len(src), int((df.member == "MARKED").sum()),
             int((df.member == "UNMARKED").sum())))
    print(df.groupby("member")[["pool", "win", "runner", "margin", "share"]].median().to_string())
