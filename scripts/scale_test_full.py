#!/usr/bin/env python3
"""Full-battery scale test: logits + beam_words for 32B and 70B.
...
def main():
    ...
    models = []
    if args.scale in ("32b", "all"):
        models += [(m, "/workspace/logits_32b") for m in MODELS_32B]
    if args.scale in ("70b", "all"):
        models += [(m, "/workspace/logits_70b") for m in MODELS_70B]
