...
date: 2026-07-27
role: finding
verification: "RE-VERIFIED 2026-07-27 against frozen final history (HEAD 5c4fa09; pinned at f4b7810, an ancestor -- appends do not orphan). All 18 commit citations in findings/ and docs/ resolve and are ancestors; see docs/citation_inventory.md and scripts/verify_citations.py. History of the defect: the original 2026-07-27 audit pass rested in part on a FABRICATED commit citation for the phrase committed before running; the real commit is 2b58732 and the fabrication is disclosed on the finding face. The ORDERING property of that citation is verified FORENSICALLY: the gate script commit 2b58732 (11:30:05) precedes the commit of its own output, 5e157c7 (11:32:39), by 2m34s, and an output cannot be committed before it is produced -- so the run falls after the script commit, on two immutable commit dates. An earlier version of this field claimed that evidence had been destroyed and the support was merely testimonial; that was wrong in the conservative direction and is withdrawn (lacan). Diffing the committed output against the current file shows the results identical, provenance key aside, so later re-runs reproduced the original exactly."
