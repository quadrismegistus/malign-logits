# Registration D v5 — the definitive displacement-site test, all three dimensions

A DELTA on frozen `registration_c_delta_v6.md` `06f0272d7f21b901`, which governs
everything not named here. **v1-v3 and v4 `1bd4ad8bbf461282` are SUPERSEDED.** v4's H1-signed arm carries over VERBATIM and is untouched by this amendment — it was designed before any unblinding and its primary status is preserved:
...
## §D0 THE DECLARED QUAD
    ...
    POPULATION   the post-construct-read survivors of the 188-pair pool
                 (`pair_drafts/EXCLUSIONS.json`). THE UNIT IS THE PAIR. A pair
...
## §D6e POWER, ON MEASURED INPUTS

    units = 188 pairs x 19.5 pair-cells x 41.7% admission = 1,529

    arm                     k_paired    MDE    realistic E   margin
    AROUSAL                   0.836*   0.0214     0.100       4.7x
    VALENCE-EXTREMITY         0.836    0.0214     0.025       1.2x
    DOMINANCE-EXTREMITY       0.707    0.0181     0.025       1.4x
    H1 SIGNED                 0.836*   0.0214     0.025       1.2x

    * AROUSAL's and H1-SIGNED's k_paired are NOT MEASURED. Valence's is used as
      a conservative stand-in and MUST be re-measured at the §D4 re-verification.

**All four detect at +2 SE on k.** `k_paired = 0.836 (val) / 0.707 (dom)`,
measured on **six displacement-selected pairs** ([1627]); **the selection's
direction is unknown and plausibly optimistic**, and it settles when the
population freezes at its hash.

**The pairing factor is `2*sqrt(1 − rho)`, not 2** — a paired statistic beats an
unpaired one exactly when its members correlate, and the measured k implies
`rho = 0.55` ([1626]/[1629]).
