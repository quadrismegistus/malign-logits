...
|---|--------|--------|--------|
| 1 | Alignment redistributes rather than deletes the transgressive lexicon: suppressed probability mass migrates within the distribution (kill 2.09→4.65 bits, OLMo). | F01 | PENDING — F01 audit day scheduled (findings-audit-schedule); the claim's components below are verified even where the file is not |
| 2 | The redistribution is ~92% genuine, not renormalisation artifact: on amber's target edge, 7.7% of the amplified set fails an exact full-vocabulary null, measured where 39.6% could have failed (19.5% conditional failure). | Tier-1 v2, docket [522]/[537] | VERIFIED (two seats, exact-count concurrence) |
| 3 | The mass concentrates: the top recipient takes 30-41% of gained mass, and independent families agree on its identity (scream top riser in 24/45 families at token level; 13/14 rising at word level, kill falling 12/14). | [515]/[522]; [713] | Token-level: VERIFIED with tokenizer-comparability scope ([654].1 re-denomination pending). Word-level: PENDING malign's anger audit ([714].2) |
| 4 | Direction of movement is largely shared across families (typical 3:1 majority) with family-structured dissent (16% same-family-different-scale vs 39% cross-family). | [715]/[716]/[717] | VERIFIED (both computations, two seats); the struck third clause (sexual exception) does NOT travel ([723]/[727]) |
| 5 | The faller-riser relation is interpretive, not geometric: four similarity instruments (WordNet, contextual cosine, inverted syntagmatic, embedding percentile) all fail the visible sites; blind judgment reads the relation instantly. | [625]/[639]/[640]; smoke tests [686]/[700] | VERIFIED as instrument-failure record; the positive characterisation (attenuation / speech-act shift) is PENDING the annotation run |
| 6 | The operation is slot-sensitive: where the grammar admits both plan and discharge, alignment chooses discharge; where it admits only physical acts, it shifts register; at referent slots it retreats along adjacency; under a cessation operator it edits the outcome. | [694]/[695]/[696]/[700]; RH's site-type notes (findings-then-and-now) | PARTIALLY VERIFIED (fist/voice control blind-coded [700].4; REF structure measured [696]; full stratified test pending annotation) |
| 7 | Category-specific targeting holds at liminal sites and fails at explicit ones, where the drain is largest but undifferentiated. | F40 (refining F06) | F40 is B/unaudited — audit scheduled behind the draft-cited findings |
| 8 | The operation installs almost entirely at SFT: base→SFT carries a median 72% of word-level distributional movement (2.58x DPO), uniformly across content categories — a level, not a contrast; it licenses nothing about repression being SFT-specific. | [680]/[682]/[683] | VERIFIED (two seats, exact reproduction; scope fused) |
| 9 | Repression precedes displacement in training: the model learns what it cannot say before it learns what to say instead. | F04 | PENDING — F04 audit day scheduled |
