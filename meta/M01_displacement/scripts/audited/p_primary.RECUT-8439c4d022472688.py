...
def main():
    ...
    if "thinking" in fields or "reasoning" in fields:
        raise SystemExit(
            "REFUSING: the stash key CARRIES a thinking field on %d of %d keys.\n"
            "[4210]'s forced-set disposition was written for a key that CANNOT "
            "express it — every item with a pre-existing entry runs cache-cold, "
            "because a thinking-on entry and a thinking-off call would produce "
            "the SAME KEY and be indistinguishable.\n"
            "**THAT PREMISE IS NOW FALSE FOR THESE KEYS**, and the disposition "
            "must be RE-DERIVED rather than inherited: where the key carries "
            "thinking state, a pre-patch entry cannot occupy a post-patch key "
            "and forcing buys nothing. Where it does not, [4210] still holds.\n"
            "This gate refused nothing on the runs of 2026-08-04 ONLY because "
            "it sampled one key of two shapes."
            % (sum(n for sh, n in shapes.items() if "thinking" in sh),
               sum(shapes.values())))
