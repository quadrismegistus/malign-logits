...
**THE PARSE DISPOSITION, DECLARED BECAUSE IT IS THIS PASS's OWN SUBJECT.**
An annotation that will not parse is a coder FAILING TO PARTICIPATE, which is
exactly the degeneracy being measured — so a silent drop would understate
degeneracy for the coder most likely to have it, and the roster decision rests
on this output. Three outcomes are counted and PRINTED PER CODER, never merged:

    JSON      `json.loads` succeeded
    REPR      `json.loads` failed and `ast.literal_eval` succeeded — a
              serialisation difference, not a coder failure. **These rows
              ENTER the rates**, because the coder answered.
    FAIL      neither succeeded. **Excluded from the rates and reported as
              NON-PARTICIPATION**, which is a finding about the coder.

"These rows are unparseable" and "these rows are repr" are different facts and
this pass converts neither into the other silently.
"""
import ast
import collections
import json
...
def main():
    ...
    def parse(val):
        """-> (annotation | None, kind). Never silently drops. See the
        docstring's PARSE DISPOSITION: a coder that cannot be parsed is a
        coder that did not participate, and that is a measurement here."""
        try:
        ...
        try:
            ...
            return (v, "REPR") if isinstance(v, dict) else (None, "FAIL")
        except Exception:
            return None, "FAIL"

    # item -> {model: annotation}; the item key carries NO side information
    by, parse_kind = {}, collections.defaultdict(collections.Counter)
    for pr, a, b, model, val in zip(df.key_prompt, df.key_a, df.key_b,
                                    df.model, df["_value"]):
        ann, kind = parse(val)
        parse_kind[model][kind] += 1
    ...
    print()
    print("PARSE CENSUS PER CODER — a coder that cannot be parsed is a coder")
    print("that did not participate, and that is this pass's own subject.")
    print("  %-28s %8s %8s %8s" % ("coder", "JSON", "REPR", "FAIL"))
    for m in ROSTER:
        k = parse_kind[m]
        print("  %-28s %8d %8d %8d%s"
              % (m, k["JSON"], k["REPR"], k["FAIL"],
                 "   <-- NON-PARTICIPATION" if k["FAIL"] else ""))
    print("  REPR rows ENTER the rates (the coder answered); FAIL rows do not.")
    print()
