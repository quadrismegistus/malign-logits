"""Generate contradiction completions for a single pair."""
import torch
...
if __name__ == '__main__':
    ...
    print(f"{'stage':>6s}  {'JS->A':>7s}  {'JS->B':>7s}  {'JS->blend':>9s}  {'ratio':>6s}  {'bias':>7s}")
    print('-' * 55)
    ...
    print(f"\n--- man/woman for reference ---")
    mw = [p for p in Psyche.DEFAULT_CONTRADICTIONS if p['name'] == 'man/woman'][0]
    for layer_name, layer in layers:
        logits_a = layer.logits(mw['prompt_a'])
        logits_b = layer.logits(mw['prompt_b'])
        logits_ab = layer.logits(mw['prompt_ab'])
        n = min(logits_a.shape[-1], logits_b.shape[-1], logits_ab.shape[-1])
        p_a = torch.softmax(logits_a[:n].float(), dim=-1)
        p_b = torch.softmax(logits_b[:n].float(), dim=-1)
        p_ab = torch.softmax(logits_ab[:n].float(), dim=-1)
    ...
    print(f"\n{'='*70}")
    print(f"  mother/father generations")
    print(f"{'='*70}")
