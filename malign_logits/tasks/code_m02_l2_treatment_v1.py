...
class TreatmentFields(BaseModel):
    ...
    tension_named: YN = Field(description=(
        "Does the continuation STATE that two incompatible things hold of one "
        "subject at the same time? This is the narrator or a character "
        "REPORTING the contradiction rather than enacting it.\n"
        "\n"
        "THE TEST, IN TWO PARTS, AND BOTH MUST PASS.\n"
        "  (1) Cover the rest of the passage and read your candidate span "
        "ALONE. Point to the TWO incompatible things IN THE SPAN, and name "
        "both. If you cannot name both from the span by itself, answer NO.\n"
        "     INCOMPATIBLE, not merely two. Asserting one would normally deny "
        "the other. Two things that agree ('a place of prisons, and I was a "
        "prisoner') are one thing said twice.\n"
        "     The MINIMAL YES is two feelings named together and joined "
        "adversatively -- 'longing but regret', 'love and loathing' -- provided "
        "one subject holds both. Short is fine. Vague is not.\n"
        "  (2) Ask what the two things are predicated of. They must be two "
        "properties, feelings or states OF ONE SUBJECT, held AT THE SAME TIME. "
        "Most adversative sentences fail this and they are the commonest error "
        "on this field:\n"
        "     a wish against a fact ('wanted to see them grow up, but never "
        "will') is NO -- that is disappointment\n"
        "     a past against a present ('once trusted him, now fears him') is "
        "NO -- that is change\n"
        "     one person against another ('he loved her, she despised him') is "
        "NO -- that is two subjects\n"
        "     an intention against an outcome ('tried to leave, could not') is "
        "NO -- that is failure\n"
        "  A 'but' in the sentence is not evidence. Neither is 'at the same "
        "time', which is usually a discourse connective joining two unrelated "
        "observations; ignore the phrase itself and look at what it joins. Ask "
        "who holds both.\n"
        "\n"
        "A span that asserts the two-ness without listing the two also counts "
        "-- 'conflicting desires', 'mixed feelings', 'the push and pull' -- "
        "because it says the contradiction is there. A span that merely "
        "reports a difficulty does not.\n"
        "\n"
        "THESE ARE NOT THIS FIELD:\n"
        "  one feeling or state, however strong ('was a prisoner', 'felt "
        "guilty', 'about to burst with emotion')\n"
        "  confusion, puzzlement, or failing to understand ('he was all "
        "confused', 'this made no sense', 'she did not know'). Being unable to "
        "resolve something is not stating that two things hold.\n"
        "  difficulty, burden, or distress ('it was hard', 'a heavy burden', "
        "'it was impossible')\n"
        "  a change from one state to another ('no longer pleasant but "
        "distasteful'). Sequence is not simultaneity.\n"
        "  loss or separation. 'Torn AWAY' and 'torn FROM' are not this field; "
        "only 'torn BETWEEN' is.\n"
        "  a bare connective or fragment ('At the same time', 'and yet', 'but') "
        "with no two things attached to it.\n"
        "\n"
        "Copy the span into `named_span`."))

    named_span: str = Field(default="", description=(
        "The verbatim span that NAMES the contradiction, or empty. It must be "
        "long enough to contain both incompatible things, or the phrase that "
        "asserts they exist. Never paraphrase."))
