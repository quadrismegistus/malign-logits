export const meta = {
  name: 'm02-opus-guilt-pathology-zh-stage-a',
  description: 'Stage A of zh guilt round 2 ([5569] design): fresh Opus readers re-read the SAME 800 zh continuations under the RULED rubric ([5567] boundaries), blind to round 1',
  phases: [
    { title: 'Read', detail: '8 agents x 100 zh passages + two controls mini-batches (batch_09 round-1 controls, batch_10 ruled-boundary controls), two independent verdicts each' },
  ],
}
...
Write JSON to \`${DIR}/out2_guilt_${name}.json\`:
...
const results = await parallel(BATCHES.map(([name, c]) => () =>
  agent(TASK(name, c), {label: `guilt2:batch_${name}`, model: 'opus'})))
