...
**Every arm is a paired contrast over the 2,722 matched pairs (§R1), tested PER CODER, with McNemar's statistic on the discordant pairs evaluated against §R3's CLUSTER-WISE SIGN-FLIP NULL over the 1,710 keys.** P registered the plain exact binomial on independent pairs; **under (b) the pairs are clustered and the null carries it, so this is P's estimator with its randomisation changed and not P's test.**
...
**A coder that never assigns a family cannot dissent from a claim about it; it can only fail to support one. Counting that as a dissent silently converts a 3/3 into a 2/3 and no reader can tell it from genuine disagreement.** Registration P printed `p=0.75000 no` for a coder with **2 discordant pairs of 93** and never printed the 2.
    ...
    **THE FLOOR IS ON DISCORDANT KEYS, NOT DISCORDANT PAIRS, BECAUSE THE
    NULL FLIPS KEYS.** §R3's randomisation has **2^K** points, where **K =
    the number of keys containing at least one discordant pair** — so the
    smallest attainable p is `1 / 2^K` and a floor stated on pairs does not
    bound it.
      ...
      TWO-SIDED  (both family arms)   2/2^K  < 0.0167  ->  **K >= 7**
        K=6 -> best-case p = 0.03125  CANNOT reach at any outcome
        K=7 -> best-case p = 0.01562  can reach

    **A pairs-floor would admit exactly the arm this section exists to
    exclude:** a coder with **30 discordant pairs concentrated in 4 keys**
    clears any n_disc >= 6 rule and **cannot beat p = 0.0625 at any
    outcome.** The keys-floor subsumes the pairs-floor — every discordant
    key holds at least one discordant pair, so `K >= 6` implies
    `n_disc >= 6` — so there is **ONE floor, on clusters.**
    ...
    **A coder below its arm's floor is reported NON-DISCRIMINATING ON THAT
    ...
    the coders that could have answered, with the excluded coder, its
    discordant-pair count AND its discordant-key count NAMED in the same
    sentence.

    **THE FLOOR NEVER CONVERTS A GENUINE DISSENT INTO A NON-COUNT.** A
    coder ABOVE its floor that fails to confirm remains a failed
    confirmation and is counted as one. The floor separates *could not
    answer* from *did not confirm*, and does nothing else.
