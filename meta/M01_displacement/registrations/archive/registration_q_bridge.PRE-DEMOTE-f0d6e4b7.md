...
**H1 — SUBSTITUTION AT SITES.** `tail_excess` differs between the marked and unmarked members of a minimal pair.
    ...
    TEST       paired, sign-flip null, TWO-SIDED, `p < 0.01`
...
**H2 — SUBSTITUTION AT SITES, IN THE GENERAL CORPUS.** `tail_excess` differs between `nonpair_transgressive` and `nonpair_neutral`.
    ...
    TEST       paired over clusters, TWO-SIDED, `p < 0.01`
    FLOOR      a cluster enters only with **>= 10 ANALYSED cells on BOTH
               ...
               **THE VALUE IS ARBITRARY; THE EXISTENCE OF THE FLOOR IS NOT,
               ...
               and 1 cells does the rest:
                   ...
                   floor >= 10   k=33   SE 0.001305   80% MDE **0.00446**
                   no floor      k=34   SE 0.002317   80% MDE **0.00792**
                                                      — **1.78x worse**

               **Both figures are at the registered two-sided alpha 0.01**
               (multiplier 3.4175, the FIVE-way split). An earlier draft quoted 0.00366 and
...
**H6 — THE NORM SIGNATURE IN THE PAIR FRAME, TESTED.** `A_|valence|` differs between the marked and unmarked members of a minimal pair.
...
**ALPHA 0.05, SPLIT FIVE WAYS — `p < 0.01` EACH — ACROSS Q's FIVE TESTED ARMS (H1, H2, H4, H5 and H6).** D2's split form, extended twice: once when H4 entered on RH's word, and again when H5 and H6 entered to complete the 3x2 grid RH's occasioning question asks for.
    ...
    **THE COST OF THE TWO NEW ARMS, PRICED BEFORE THEY WERE ADMITTED** —
    the rule H4's own entry established:
      ...
      5 arms  alpha 0.010000  z 2.5758  **multiplier 3.4175 = 2.5758 + 0.8416**
      **every existing MDE grows 5.6%:**
        H1  0.00183 -> **0.00193**   (2.48% -> **2.61%** of N's −0.0738)
        H2  0.00422 -> **0.00446**
        H4  0.00554 -> **0.00585**
...
**COMPUTED FOR THE ESTIMATOR §Q3 REGISTERS — the CLUSTER, paired within cluster (the PAIR for H1) — and at 80% power**, which is this campaign's convention (D2 §A7: *"80% power, simulation at realized pair-count and variance, RAW scale"*).
    ...
    H1  tail_excess, k=684       0.000565   **0.00193**   0.01    **N's own**, spent, BLIND
    H2  tail_excess, k=33        0.001305   **0.00446**   0.01    **N's own**, spent
    H4  departed, k=33           0.001711   **0.00585**   0.01    **L's FOUND PROSE**, borrowed
    **H5  departed, k=684        PENDING    PENDING**     0.01    **N's own**, spent, BLIND
    **H6  A_|valence|, k=626     PENDING    PENDING**     0.01    **N's own**, spent, BLIND
    H3  A_|valence|, k=32        0.013181   **0.03693**   0.05    **O's**, borrowed
                                                                  (CORPUS arm only; estimated;
                                                                   consumes no alpha)

    **H5's AND H6's ROWS ARE PENDING AND SAY SO RATHER THAN CARRYING A
    PLACEHOLDER.** Both are PAIRED, so each needs `sd(d_i)` — and the SD of
    ...
    ones is self-verifying in the only way a blind instrument can be.
      **MDE(H5) = 3.4175 x sd / sqrt(684)**
      **MDE(H6) = 3.4175 x sd / sqrt(626)**

**EVERY ROW NAMES ITS ALPHA AS WELL AS ITS DISPERSION, BECAUSE THE TABLE MIXES BOTH.** H1, H2, H4, H5 and H6 are at the registered two-sided **0.01**, multiplier **3.4175** = 2.3940 + 0.8416, the exact three-way split 0.05/3. **H3 is at two-sided 0.05** — the conventional statement for an arm that consumes no alpha and runs no test; at 0.01 it would be **0.04505**, and H3's conclusion is the same at either, which is why the looser figure is quoted and labelled rather than hidden.
...
**THE ROW FILLED FROM A FORMULA THAT WAS IN THIS TEXT BEFORE `sd_d` EXISTED — THE NUMBER FILLED A SLOT RATHER THAN CHOOSING ONE:**
    ...
    **MDE(H1) = 3.4175 x sd_d / sqrt(k)**,  k = 684 stems

    **sd_d = 0.014765**,  **k = 684**   — the pass's entire output
    **MDE(H1) = 3.4175 x 0.014765 / sqrt(684) = 0.00193**, checked at a
    second seat. **Against N's pooled level of −0.0738 that is 2.61%.**
    ...
    **3.4175 = 2.5758 + 0.8416** — the TWO-SIDED z at **alpha 0.05/5**
    (§Q3.1's FIVE-way split, `p < 0.01` exactly) plus the 80%-power z.
    **An earlier cut used 3.2356 at the three-way split; H5 and H6's entry
    moved it, and every MDE above is re-stated at the new multiplier.** **The two constants it is not, and both have been used
...
**"SIGNIFICANT" IN EVERY BRANCH BELOW MEANS `p < 0.01`** — §Q3.1's FIVE-way split of alpha 0.05 over H1, H2, H4, H5 and H6, **repeated here at the point of use.** A reading rule that names its threshold only in another section is a reading rule whose first reader supplies the number, and the first draft of this registration named it in no section at all.
...
**Both are within-pair contrasts on the same unit as H1 and read the same way.**
    ...
    **H6 SIGNIFICANT**
        **CONFIRMED — the norm signature differs at the site relative to
        its twin.** The corpus half stays ESTIMATED (H3), so the norms row
        has a tested twin cell and an estimated corpus cell, **and no
        norms ratio may be quoted as a tested quantity.**
    **EITHER NULL**
        **NOT SUPPORTED, quoted as a BOUND at the realized dispersion**,
        never as an absence.
    **EITHER SIGNIFICANT IN THE UNEXPECTED DIRECTION**
