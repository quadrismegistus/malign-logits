...
#: **AMENDED AT [4572]: A CITATION MUST GOVERN THE ARM, NOT MERELY EXIST.**
#: [4501] required each sign to cite a frozen line rather than an argument.
#: It required the citation to EXIST and never that it GOVERN. An earlier
#: cut gave H2 the sign at SQ3 L314 -- frozen and pre-registered -- which
#: sits in **H1's** block, two lines above H2's heading, and reads
#: *"Substitution predicts MARKED to be more negative."* **Its subject is
#: the marked member of a minimal pair; H2 contrasts nonpair_transgressive
#: against nonpair_neutral and has no marked member.** H2 then printed
#: "significant IN THE WRONG DIRECTION -- REPORTED AS A REVERSAL": a
#: verdict against an expectation Q never registered for it.
#:
#: **THE SAME IMPORT WAS LATENT ON H4.** L845 says in its own words that F
#: and G found the shape *"inside the pair frame"*, and H4 is a corpus
#: contrast. H4 came out null, so it never fired -- **a latent defect in a
#: branch that did not fire is the one no verification finds.**
#:
#: Both corpus arms now declare NO direction, and that is the finding
#: rather than a gap: **Q registers a directional expectation for the PAIR
#: frame and none for the corpus.** SQ5.1's frame reversal is unaffected --
#: its fourth branch is RELATIONAL (both significant, opposite signs) and
#: needs no registered direction for either arm.
NO_DIRECTION = "Q REGISTERS NO DIRECTION FOR THIS ARM"
DIRECTION = {
...
#: each arm's own block in the frozen text, for the gate
ARM_BLOCK = {"H1": (261, 315), "H2": (316, 357), "H4": (358, 378),
             "H5": (379, 393)}
...
def branch_sentence(arm, obs, sig):
    ...
    **THIS IS THE FUNCTION 9.3 EXERCISES.** It is at module level, and the
    ...
    if want == 0:
        #: no registered direction: report significance and say so.
        if not sig:
            return ("NULL — quoted as a BOUND, never as an absence",
                    None, False, cite, kind)
        return ("significant. **%s** — no verdict about direction attaches, "
                "and \"wrong direction\" is not available as a reading. "
                "SQ5.1's frame reversal is RELATIONAL and unaffected." % kind,
                None, False, cite, kind)
...
def gate_direction_citations():
    """**A CITATION MUST GOVERN ITS ARM** ([4572]). An arm claiming a
    direction must cite a line inside its OWN block in the frozen text;
    an arm citing a neighbour's line, or a sibling registration's, must
    declare it has no direction of Q's own."""
    bad = []
    for arm, (sign, cite, kind) in sorted(DIRECTION.items()):
        if sign == 0:
            if kind != NO_DIRECTION:
                bad.append("%s: sign 0 without a no-direction declaration" % arm)
            continue
        m = re.search(r"L(\d+)", cite)
        if not m:
            bad.append("%s: cite %r names no line" % (arm, cite))
            continue
        line, (lo, hi) = int(m.group(1)), ARM_BLOCK[arm]
        if not (lo <= line <= hi) and "prior Q CITES" not in kind:
            bad.append("%s: cites L%d, outside its block [%d, %d], and does not "
                       "declare the citation a sibling prior" % (arm, line, lo, hi))
    if bad:
        refuse("direction citations do not govern their arms: " + "; ".join(bad))
    return sorted(a for a, v in DIRECTION.items() if v[0] == 0)
...
def main(argv=None):
    ...
    nodir = gate_direction_citations()
    print("  direction citations GOVERN   PASS (%s declare none)" % ", ".join(nodir))
    ...
    for arm in ("H1", "H2", "H5", "H4"):
        ...
        res[arm] = {"mean": obs, "k": len(d[arm]), "p": p, "p_mc_se": mc,
                    ...
                    "direction_governs_arm": DIRECTION[arm][0] != 0,
