...
TOL is 0.5 in log2, a factor of 1.41. v4's match quality is median 0.056 with
87.1% inside a factor of 1.4, so the gate is set at the existing distribution's
own 87th percentile rather than at a number chosen to produce an outcome.
...
DR_PATH = os.path.join(ROOT, "meta", "M04_syntagmatic", "results",
                       "dialogue_rate.json")
DR_TOL = 0.5          #: log2; a factor of 1.41, v4's own 87th percentile
...
TOLS = [0.0, 0.05, 0.10, 0.15, 0.25, 0.50, 1.00]
SWEEP = {t: [] for t in TOLS}
...
def main():
    ...
    for pr in pairs:
        ...
        for r in rows:
            ...
            #: SWEEP: the same selection at every tolerance, one walk.
            base = (min(cands, key=lambda w: abs(math.log2(m.post[w] / qf0)))
                    if cands and qf0 > 0 else None)
            for _t in TOLS:
                _near = ([w for w in cands
                          if abs(math.log2(m.post[w] / qf0)) <= _t]
                         if cands and qf0 > 0 else [])
                _w = (min(_near, key=lambda w: abs(dr_of(w) - dr_of(f)))
                      if _near else base)
                if _w is not None:
                    SWEEP[_t].append((abs(math.log2(m.post[_w] / qf0)),
                                      abs(dr_of(_w) - dr_of(f)),
                                      bool(_near), dr_of(_w), dr_of(f)))
            nm = base
            dr_matched = False
    ...
    print("\n" + "="*74)
    print("TOLERANCE SWEEP -- probability gate against dialogue-rate objective")
    print("  TOL is the HARD bound on |log2(Q_matched/Q_faller)|.")
    print("  TOL=0 reproduces the existing rule (probability only).\n")
    print("  %-6s %11s %11s %11s %11s"
          % ("TOL", "med |log2|", "%<=1.4x", "med d_rate", "% in gate"))
    for t in TOLS:
        v = SWEEP[t]
        if not v: continue
        print("  %-6.2f %11.3f %10.1f%% %10.3fpp %10.1f%%"
              % (t, _st.median(x[0] for x in v),
                 100*sum(1 for x in v if x[0] <= 0.5)/len(v),
                 100*_st.median(x[1] for x in v),
                 100*sum(1 for x in v if x[2])/len(v)))
    print("\n  MARGINAL dialogue rate of the matched arm at each TOL")
    print("  (faller marginal is fixed: median %.2f%%, share>20%% %.1f%%)"
          % (100*_st.median(x[4] for x in SWEEP[TOLS[0]]),
             100*sum(1 for x in SWEEP[TOLS[0]] if x[4] > 0.20)/len(SWEEP[TOLS[0]])))
    print("  %-6s %13s %13s" % ("TOL", "med rate", "share >20%"))
    for t in TOLS:
        v = SWEEP[t]
        if not v: continue
        print("  %-6.2f %12.2f%% %12.1f%%"
              % (t, 100*_st.median(x[3] for x in v),
                 100*sum(1 for x in v if x[3] > 0.20)/len(v)))
    print("="*74)
