...
**H2 — SUBSTITUTION AT SITES, IN THE GENERAL CORPUS.** `tail_excess` differs between `nonpair_transgressive` and `nonpair_neutral`.
    ...
    FLOOR      a cluster enters only with >= 10 analysed cells on BOTH sides;
               clusters below the floor are REPORTED, never dropped silently
...
Computed from within-cluster residual SD measured on **spent, published data** — N's `tail_excess` (MSW SD 0.0602) and O's `A_|valence|` (MSW SD 0.4971) — against the cell counts in §Q1.3.
    ...
    arm                                       MDE      the effects at issue
    H2  tail_excess, general corpus       **0.0022**   pooled level −0.0738
    H3  A_|valence|, general corpus       **0.0226**   C's general +0.025,
                                                       D2's site increment +0.015

**H2 IS POWERED BY A WIDE MARGIN** — it can detect a partition difference of 3% of the pooled level.

**H3 IS MARGINAL AND SAYS SO** — detectable at C's effect size, invisible at D2's. **This is why H3 estimates and does not test.**
