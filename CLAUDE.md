...
- **Entropy mediation: DIRECTION HOLDS, MAGNITUDE UNSOURCED.** The liminal−explicit entropy gap reproduces exactly (1.315 nats at n=9, 1.294 at n=8). The booked slope of +0.0187/nat does not: mean-of-within-family-OLS gives +0.0148/nat and pooled OLS +0.0174/nat on the same n=9 data, so the "~91% explained" figure rests on a slope computation nobody can currently identify. Under the two methods that *are* reproducible the share explained is **67–79% at n=8** (72–84% at n=9). Cite the entropy effect as substantial and the exact share as pending re-derivation; do not cite 91%.
