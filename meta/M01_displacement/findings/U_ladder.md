...
The words also stay the same — **for three of the four arms, and this paragraph used to say it on the strength of the one where it is truest** (corrected 2026-08-14, [6121]). `full` against `no-safety` has faller Jaccard 0.534 against 0.02 to 0.04 between ladder rungs, and `no-math` and `no-persona` sit with it at 0.528 and 0.522. **`no-wildchat` is at 0.340**, and against each of the other three it is 0.294 to 0.303 where they sit at 0.486 to 0.563 with each other — **two groups with no overlap.** So: change the training corpus and you get the same operation on the same words, *except* when the corpus removed is WildChat; change the rung and you never do. **The original sentence cited 0.534 as "among the highest in the fan", which was true and did the work of hiding the spread it was highest in.**
