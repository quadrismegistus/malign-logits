...
| [crosslingual_arms.md](findings/crosslingual_arms.md) | ALIGNMENT REDUCES TRAJECTORY DRIFT IN CHINESE AS IN ENGLISH, same 25 pairs, same corpus, same rung and -- after RH corrected the fence -- the SAME MATCHED PROMPTS (97 keys, zero zh-only): zh -0.0314 (4/21) and en -0.0205 (1/24) on total_drift, both stronger on mean_drift, all four cells surviving n_sents matching, with the confound running AGAINST the finding. The language DiD is NULL on every construction -- p .23 to 1.0 pooled and **p .69 on both metrics with MATCHED PROMPTS** (1,901 pair-key units), the strongest form, holding topic/construct/role by construction. First arm effect this campaign has measured on Chinese GENERATED TEXT. Two population questions handled as declared sensitivities rather than swaps (28-pair `ambiguous`-flag rerun, unchanged and stronger on count). | draft; second-seated ([5845]); matched-prompt DiD single-pass |
| [composition_not_level.md](findings/composition_not_level.md) | M01 and M06 are ONE OPERATION AT TWO SCALES. The passage surprisal drop is **COMPOSITION** (-1.310 of -1.285 nats/word) and not **LEVEL** (+0.025) -- alignment changes WHICH words appear, not what they cost in context, i.e. selection and not combination, converging with the propagation slope. Composition tracks M01's **DIRECTIONAL** displacement (partial rho -0.276, 36/36 pairs) and **NOT volatility** (+0.008, 18/36 = exact chance; `the` is non-still in 30.5% of cells with -3.3% direction and disqualifies itself). THE COST CLAIM IS A CONTRAST, NOT A LEVEL: on base text the aligned model finds nearly everything costlier, words M01 never measured most of all (+0.614 at 71.7% of tokens), so fallers sit BELOW average -- what survives is that **at matched aligned probability, words pushed DOWN cost 0.347-0.444 nats more in context than words pushed UP** (34/35, 35/35), which is the demoted-vs-merely-improbable separation M04's A_post_utterance_shock declared impossible for want of a matched corpus. NOT syntagmatic damage: the chain is held constant by construction. One mechanism measured and REFUTED (promotion is MORE consistent than demotion, not less). Withdrawn: the +0.284/+0.320 correlations, all classified-mover numbers, the 33-pair figures. | draft; single pass, one seat, ungraded |
| [drift_metric_audit.md](findings/drift_metric_audit.md) | AUDIT of the drift metric family, four measured defects: `total_drift` is ORDER-INVARIANT (dispersion, not trajectory -- verified by permutation); it is 92% NOISE at passage level (ICC .082 against surprisal's .371), so a median split on it classifies passages near-randomly and Cohen's d against passage SD divides by mostly measurement error (surprisal:drift is 4.4x / 2.1x / 1.3x depending on standardisation); **DIRECTEDNESS IS SENTENCE COUNT** (Spearman -.923, R^2 .795 against 1.681/n; every corpus converges within a fixed n), so the shape reading built on it is void; and the 75-word truncation analyses 46% of each generation while RETAINING FEWER passages (86.1%) than no truncation (95.1%). Use `mean_drift`, the within-passage ordering contrast for shape, and classify pairs not passages. | draft; exploratory, single pass |
