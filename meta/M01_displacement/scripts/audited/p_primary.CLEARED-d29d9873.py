    ...
    for _f in ("largeliterarymodels/providers.py", "largeliterarymodels/llm.py"):
        ...
        if here != pinned:
            raise SystemExit(
                ...
                "matches and THE CODE DOES NOT — an edited "
                "`deepseek_thinking_default` keeps its name, keeps the field "
                "arriving, and leaves thinking ON."
                % (_f, here[:12], pinned[:12], EXPECTED_HEAD))
