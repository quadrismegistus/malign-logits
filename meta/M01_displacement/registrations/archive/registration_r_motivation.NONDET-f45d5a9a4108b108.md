...
## §R1 POPULATION, MATCHING, AND THE ONE FILTER
    ...
    **AND 2,722 IS A PRE-FILTER CEILING, NOT THE TESTED n.** §R1.1's
    content-word filter passes **2,599 of 4,443 ITEMS (58.5%) and excludes
    1,844** — measured by the freeze-precondition pass. A pair enters only
    if BOTH members pass, so **the ACHIEVED pair count is materially lower
    than 2,722 and is NOT KNOWN HERE.**
    **The achieved n and the per-side filter rates are the producer's FIRST
    outputs, and they are deferred on purpose:** computing the pair-level
    count now requires the per-side pass rate, and *"do risers and controls
    differ in content-word rate"* is precisely the confound §R1.1 exists to
    remove. **Measuring it before this document freezes would spend the
    thing the filter protects.**
    **A reader of this section alone would otherwise expect 2,722 tested
    pairs and meet materially fewer.**

    **AND THE ACHIEVED n IS NOT MERELY UNKNOWN-UNTIL-RUN — IT IS A RANDOM
    VARIABLE.** Registration P's own re-annotation measured the coders
    against themselves: **341 items called twice under identical parameters,
    91 differed — 26.7%** — and the content-word flags are inside that.
    **`a_is_content_word` and `b_is_content_word` each moved 13 of 341 =
    3.81% per flag per call.**

    §R1.1's filter rests on **SIX** such judgments per item (three coders x
    two words) and **TWELVE** per pair, since both members must pass:

        2 flags   0.9619^2  = 0.9252  ->  7.5% would drop
        6 flags   0.9619^6  = **0.7920**  -> **20.8% of passing ITEMS**
        12 flags  0.9619^12 = **0.6272**  -> **37.3% of surviving PAIRS**
        **of the 2,599 items passing today, ~541 would not on a re-run.**

    **THOSE FIGURES ASSUME THE TWELVE JUDGMENTS ARE INDEPENDENT AND THEY
    ARE NOT.** A borderline word is borderline for every coder, so the flips
    correlate positively and **positive correlation REDUCES the
    compounding.** **37.3% is an UPPER BOUND under an assumption this design
    does not have; the true figure is lower and nobody has measured it.**
    The bound is stated with its assumption in the same breath because a
    compounded figure quoted without one is the defect class this campaign
    spent 2026-08-04 correcting.

    **IT IS NOISE AND NOT BIAS, WHICH IS THE REASSURING HALF.** The filter
    drops a PAIR if either member fails, and instability hits risers and
    controls independently and symmetrically — **it cannot favour an arm.**
    So it ATTENUATES: it costs power and widens the null's variance, and it
...
- **IT CANNOT QUANTIFY CODER-RESAMPLING UNCERTAINTY, AND ITS p-VALUE ANSWERS A NARROWER QUESTION THAN A READER WILL ASSUME.** §R3's cluster-wise sign-flip permutes the SIGNS of per-pair differences it treats as FIXED. **They are not fixed — they are ONE DRAW from coders that disagree with themselves 26.7% of the time** (Registration P, 341 items called twice under identical parameters). **R's null therefore quantifies PAIRING and CLUSTERING uncertainty and NOT coder-resampling uncertainty.** A re-run would draw different labels, a different achieved n (§R1), and a different p. **This does not invalidate the test; it names which uncertainty the test is measuring, so nobody reads the wider one into it.**
