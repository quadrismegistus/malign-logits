...
#: [4240]: the peer seat's FINAL hash. `b3e0915` is orphaned — reset and
#: rebuilt into five thematic commits — and is on no branch.
EXPECTED_HEAD = "0c0a983"
...
def gate_instrument():
    ...
    got = DisplacementRelationTask().instrument_sha256()
    if got != INSTRUMENT_DIGEST:
        raise SystemExit(
            "REFUSING: instrument digest is %s, §P2 pins %s. The rendered "
            "instrument moved — judgments from a different instrument DO NOT "
            "POOL." % (got[:16], INSTRUMENT_DIGEST[:16]))
    src = os.path.join(ROOT, "malign_logits", "tasks",
                       "code_displacement_relation.py")
    fsha = hashlib.sha256(open(src, "rb").read()).hexdigest()
    print("gate     §P2 instrument digest  %s  (Task.instrument_sha256)" % got[:16],
          flush=True)
...
def forced_set(items, model):
    """§P4/[4210]: every item with a PRE-EXISTING stash entry runs cache-cold.

    The stash key holds `max_tokens, model, prompt, schema, system_prompt,
    temperature` and NO thinking field, so a thinking-on entry and a
    thinking-off call collide. Forcing is the only disposition that needs no
    argument about when an entry was written.
    """
    from malign_logits.tasks.code_displacement_relation import DisplacementRelationTask
    t = DisplacementRelationTask()
    t.model = model
    have, seen, parsed = set(), 0, 0
...
def stash_key_fields():
    """[4210]: print the key's fields, so its blindness is on the record."""
    from malign_logits.tasks.code_displacement_relation import DisplacementRelationTask
...
def main():
    ...
    fields = stash_key_fields()
    print("\ngate     stash key fields: %s" % ", ".join(fields), flush=True)
    if "thinking" in fields or "reasoning" in fields:
        raise SystemExit("REFUSING: the stash key now carries a thinking field. "
                         "[4210]'s forced-set disposition was written for a key "
                         "that CANNOT express it; re-derive before running.")
    print("         no thinking field — the forced set is the disposition", flush=True)
    ...
    for model in ROSTER:
        ...
        t = DisplacementRelationTask()
        t.model = model
        if t.instrument_sha256() != INSTRUMENT_DIGEST:
            raise SystemExit("REFUSING: instrument moved under coder %s" % model)
...
def emit(d, sel, coded, keys, coders, roles, pins):
    ...
    def mix(rows, f):
        return dict(collections.Counter(
            (tuple(r[f]) if isinstance(r[f], list) else r[f]) for r in rows))
    ...
    tot = sum(v for k, v in im.items() if isinstance(k, str))
    ...
    print("  NARR taxonomy (main exhibit, %d items): %s" % (
        len([r for r in REAL if r["slot"] == "NARR"]),
        dict(sorted(((str(k), v) for k, v in desc["narr_relation_taxonomy"].items()),
                    key=lambda kv: -kv[1])[:6])), flush=True)
    print("  B_MILDER share %s   NOT_COMPARABLE rate %s   (§P4.1 clause 2: "
          "NOT_COMPARABLE is off-scale and excluded from the ordinal metric)"
    ...
    incomplete = {}
    for _m in ROSTER:
        n_present = sum(1 for r in cells if r["coders"].get(_m) is not None)
        n_missing = 4443 - n_present
        print("  reconciliation  %-28s %d/4443 coded  (%d missing)"
              % (_m, n_present, n_missing), flush=True)
        if n_missing:
            incomplete[_m] = n_missing
    ...
    if incomplete:
        payload_incomplete = {
            ...
            "_disposition": (
                "Per-coder annotation counts fell short of 4,443. The frozen "
                "text sets no threshold, so this producer declares one: the "
                "cells are WRITTEN (13,329 paid judgments are not discarded "
                "over a transport failure) and the PRIMARIES ARE NOT "
                "COMPUTED. §P3's test is a paired count over three coders and "
                "a short coder silently changes its denominator."),
        ...
        json.dump(payload_incomplete, open(out_i, "w"), indent=1,
                  sort_keys=True, default=str)
        os.chmod(out_i, 0o444)
        raise SystemExit(
            "REFUSING to emit verdicts: %r. The evidence is written to %s and "
            "locked; the primaries are NOT computed, because §P3's paired "
            "count over three coders has a denominator a short coder changes "
            "silently." % (incomplete, os.path.basename(out_i)))
    ...
    if os.path.exists(OUT):
        ...
        os.chmod(OUT, 0o644)
    os.makedirs(os.path.dirname(OUT), exist_ok=True)
    json.dump(payload, open(OUT, "w"), indent=1, sort_keys=True, default=str)
    os.chmod(OUT, 0o444)
