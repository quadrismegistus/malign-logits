...
**THE PER-MEASURE DENOMINATORS OF §Q1.2 ARE A READ-OUT PROPERTY, NOT A COMPUTATION PROPERTY.** `tail_excess` and `departed` are read over analysed cells and `A` over A-cells, and **that filtering happens at read-out.** A cell that is analysed-but-not-an-A-cell still has its `A` computed and then declined; it is never the case that a measure was skipped for some cells and the difference later mistaken for a null.
