# M01 — Displacement: the ledger (clause-by-clause record)

This file was `README.md` until 2026-08-03, renamed at RH's word. It is the
campaign's DETAIL layer: every clause with its full status, caveats,
supersessions and docket citations. For the reader's map — what the campaign
asks and what it has found, organised by question — see `README.md`.
...
### 1. WHERE A REGISTRATION TESTS A CLAUSE — TWO PLACES, OUT OF TEN CLAUSES AND THIRTEEN REGISTRATIONS
    ...
    clause 1  `mass-migration`
      **N**  states it in its own first paragraph: "Tests ledger clause 1
             (`mass-migration`)." 2,199 English stimuli x 44 edges, 82,775
             analysed cells, 34 clusters, one-sided sign test on
             `tail_excess`. **SUBSTITUTION CONFIRMED, 91% of cells, 34/34
             clusters agree.**
      **O**  the same claim in two languages. **H1 SUPPORTED IN BOTH ARMS**,
             under §O6's bound (all nine clusters Chinese-origin/heavy).
      L, M   bear on the RETENTION BOUNDARY rather than on migration: L
             measures that alignment loses the human's word, M adjudicates
             that eviction as **boundary proximity, not tail contraction**.
             Adjacent, not the clause.

    clause 2  `null-survival`
      **N**  **DECLINES IT, explicitly and by design**: "Clause 2 stays scoped
             as it is — verified-at-its-time, one family — and arm B is
             retired by its own test (§5)." N's §4.1 adversarial correction
             tests a THRESHOLD artifact and passes it (0.13% of cells moved,
             statistic identical to six decimals); **that is not the exact
             full-vocabulary renormalisation null clause 2 rests on**, and no
             registration has run one.

### 2. THE CLAUSE THIS CORRECTS, AND IT IS THE ONE RH NAMED

**Clause 1's status column reads "PENDING — F01 audit day scheduled."** That is true of the FINDING FILE and false of the CLAIM. **The claim was tested at full scale on the live roster by a frozen registration and confirmed.** Both facts belong in the row and only one was in it.

    **A CLAUSE'S STATUS IS ABOUT THE CLAIM. AN AUDIT DAY IS ABOUT A FILE.**
    F01's audit is still owed — the bits figure (kill 2.09->4.65, OLMo) is
    F01's and N did not test it — but "PENDING" alone told a reader the
...
### 3. THE STRUCTURAL FACT: TEN OF TWELVE REGISTRATIONS MAP TO NO CLAUSE AT ALL
    ...
    **D**    the definitive displacement-site test, three dimensions
    **D2**   the two extremity arms at sites
    **D3b**  decomposing D2's effect against pool extremity (+ Amendments A, B)
    **E**    C's H2 on the GAP stratum
    **F**    within-pair displacement RATE at transgressive sites (null)
...
**None of the ten clauses is about affect, extremity, minimal pairs, found prose or the retention boundary.** The clause list was written when the campaign was a distributional account of *how much* moves and *where*; **the registered work since is largely an account of WHAT KIND of word moves, and the table has no row for it.**
    ...
    **THE CLAUSES ARE NOT WRONG. THEY ARE OLDER THAN THE CAMPAIGN.**
    A reader consulting the clause table for "what has M01 established"
    sees ten claims of which two have registered backing, and does not see
    the eleven registrations that are the strongest evidence in the folder.

**This is a structural gap and I am not closing it by inventing clauses.** Renumbering or extending the clause list is a decision about what the campaign claims, which belongs to RH and the pen, not to a currency pass. **What this section does is make the gap visible where the table is read** — the same fix as adding the column, one level up.

### 4. WHAT WOULD CLOSE IT

- **New clauses for the affect line** (C/D/D2/D3b/E) — de-extremification general, and stronger at sites — which is section 2a of the README stated as clauses.
- **A clause for the boundary** (L/M) — alignment evicts the human's word, and the eviction is boundary proximity.
- **A clause for the crosslingual split** (O) — substitution crosses, the affective signature does not or cannot be seen.
- **Registration P**, which would give clauses 4, 6 and 7 their first registered instrument at once.
