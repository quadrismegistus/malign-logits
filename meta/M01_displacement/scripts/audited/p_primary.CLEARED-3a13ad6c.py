...
def main():
    ...
    for model in ROSTER:
        ...
        if not t.usage.no_reasoning_observed():
            raise SystemExit(
                "REFUSING after ONE item: %s reported reasoning tokens (%d). "
                "Thinking is on; the other 13,328 calls would be uncontrolled "
                "sampling with nothing in the artifact to contradict them."
                % (model, r0["reasoning_tokens"]))
        if model.startswith("deepseek/") and drops:
            raise SystemExit(
                "REFUSING after ONE item: %s dropped %r. DeepSeek "
                "ACCEPTS-AND-IGNORES, so a dropped parameter here is the "
                "silent failure shape and not the reported one." % (model, drops))
        t.usage.reset()
        for label, idxs, force in (("forced", idx_forced, True),
            ...
            got = t.map(sub, num_workers=WORKERS, force=force, verbose=True,
                        errors=errs, per_item_usage=piu)
            for j, i in enumerate(idxs):
                anns[i] = got[j]
                if j in piu:
                    per_coder_usage.setdefault(model, {})[i] = piu.pop(j)
            print("  %-8s %d items  %s" % (label, len(idxs),
                                           t.usage.summary_line()), flush=True)
        coders[model] = anns
