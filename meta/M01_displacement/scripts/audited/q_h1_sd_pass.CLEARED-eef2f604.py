...
# Public known answers ([4319]/[4321]/[4332]); any mismatch is a refusal.
EXPECT_CELLS = 82775
EXPECT_PAIRS = 684
EXPECT_PAIR_TEXTS = 1368
EXPECT_BOTH_SIDES = 24606
EXPECT_ONE_SIDE = 1253
# tulu / tulu-no-safety share base AND superego checkpoint: one measurement
# under two family labels ([4333]'s ruling, CLAUDE.md's standing treatment).
# Their shared edge duplicates every pair-member text's cell exactly once.
EXPECT_COLLAPSED = 1368
...
def main():
    ...
    # (stem, edge) -> {role: tail_excess_corrected}, edge = (base, aligned):
    # family labels collapse. A duplicate key must carry an IDENTICAL value
    # (one measurement under two family labels) and counts once; an unequal
    # duplicate is not one measurement and refuses ([4333]).
    by_key = {}
    collapsed = 0
    for c in cells:
        ...
        if role in slot:
            if slot[role] != c["tail_excess_corrected"]:
                refuse("duplicate (stem, edge, role) cells DIFFER on edge "
                       "%s -> %s — not one measurement; ruling reopens"
                       % edge)
            collapsed += 1
            continue
        slot[role] = c["tail_excess_corrected"]
    if collapsed != EXPECT_COLLAPSED:
        refuse("collapsed %d duplicate cells, not %d"
