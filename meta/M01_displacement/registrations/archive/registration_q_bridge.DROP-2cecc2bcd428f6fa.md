...
An earlier cut carried two more, on RH's word: **G re-derived at `d = 0.748`, and D2 re-derived at `A_|valence| +0.01525, A_|dominance| +0.01624`.** RH asked what they were for, and checking the sources answered it:
    ...
    **G's ARTIFACT DOES NOT CONTAIN 0.748.** `result_g_magnitude.json`
    holds `primary/statistic = 0.16854886697` (p 6.0e-05, reject) and
    `secondary/statistic = 0.01551217` (p 0.45). **A sweep of every result
    artifact finds 0.748 only as per-row cell values in N/L/M/O — noise at
    that precision across tens of thousands of rows — and as D3b's
    `mean_abs_z_weighted = 0.747691`, a POOL-EXTREMITY REGRESSOR and not a
    magnitude effect size.** The known answer was a transcription from the
    wrong registration.
