...
**AMENDED 2026-08-12 per lacan [5504], reproduced independently by both other seats ([5505], pen same morning): the MAGNITUDE RATIO is retired as an independent fact.** The marginal is a share-among-risers minus a share-among-fallers, so the deltas sum to zero within every (stratum, labeling) by construction (verified to 8.8e-15 on the committed artifact, three seats, two routes). Given that identity, if more categories land riser-skewed, the faller-skewed ones are ARITHMETICALLY FORCED to carry larger magnitudes: the 3.8x is the zero-sum identity plus the count asymmetry, not a fact about displacement. SLICE OF RECORD for the numbers below, per [5505]'s demand: stratum=ALL, the seven category labelings (TOKEN excluded), Bonferroni survivors — 206/36, means +0.00334/-0.01267, 3.79x; the ratio ranges 1.79x-5.50x across other slices of the same artifact and no single value should travel without its slice. **What survives as the finding: the COUNT asymmetry (with the granularity caveat below) and the equal-draw category-scatter test two paragraphs down — risers occupy more distinct categories at matched token counts — which is the chain-not-swap claim in its unforced form.** The paragraphs below stand as evidence for that reading; their magnitude language should not be quoted.
