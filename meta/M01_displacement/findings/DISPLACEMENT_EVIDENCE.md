...
**MARKED PENDING REBUILD, 2026-08-07 — DO NOT QUOTE THE CONCLUSION BELOW UNTIL THE PRODUCER EXISTS.** The roster query that settled this was a heredoc at malign's seat and no script on the machine reproduces it. phi-4's entropy drop of +0.0812, one of the five low-concentration values, does not reproduce from any population that can be constructed: the beam population gives +0.1391, the full 2,583-prompt population +0.0448, and the closest candidate reading is 11 percent off. Nothing computes fallers-per-site across registry pairs at all, so **both halves of the criterion are currently unreproducible.**

It is rebuildable rather than lost — registry plus `true_word_probs`, no generation needed — and that distinction is the whole difference between "wants a producer" and "unrecoverable". A rebuild is underway. **If it reproduces, the conclusion stands with a script under it; if it does not, this section changes.** Recorded here rather than only on the docket because this document is the paper-facing record and the conclusion is load-bearing: it is what retired the low-concentration caveat instead of carrying it into print.

The finding as originally reported, now awaiting its producer: The roster query ran on a criterion declared before looking (concentrates little = entropy drop < 0.10 nats; aligns strongly = fallers per site at or above the 44-pair median of 13.0; scoreable from `true_word_probs` without new generation). **Of 44 candidate base-to-superego pairs, NONE meet both.**
