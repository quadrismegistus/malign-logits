...
Five documents, all drafts at grade C, written 2026-08-11 by the registrar
seat. A-D are on ONE lineage (OLMo-3); E is the Pythia ladder, a SEPARATE
STUDY never pooled with A-D (cross-ladder comparisons only). Figures live in
`figures/fig1..fig14` (plotnine, 300 dpi) and are embedded in the findings;
every finding re-derives from the scripts named in its own header.
