    ...
    if os.path.exists(pcf):
        ...
        percell = {
            ...
            "threshold_sensitivity": {
                ">=1": "+9.5 +6.7 -4.3 -2.3   flip Q2/Q3, min NOUN cell 102",
                ">=2": "+8.1 +6.0 -5.0 -1.4   flip Q2/Q3, min NOUN cell  79",
                ">=5": "+1.4 +0.5 -3.3 -1.7   flip Q2/Q3, min NOUN cell  40",
                ">=10": "+7.9 -4.0 -10.7 -2.1  flip Q1/Q2, min NOUN cell  23",
                "_reading": "the flip point MOVES and the magnitudes swing "
                            "several-fold across an inclusion cut nobody has a "
                            "principled reason to set; at >=5, a defensible "
                            "threshold with 40 nouns in its smallest cell, the "
                            "whole contrast sits inside 3.3 points and reads "
                            "FLAT. And it is not monotone at any threshold: Q3 "
                            "is more negative than Q4 in every row.",
            },
            "verdict": "NULL WITH ADEQUATE STRATA (102-150 nouns per cell at "
                       ">=1 event, all four fire). The pooled gap is a weighting "
                       "fact about the eligibility distribution. THE "
                       "PER-STRATUM SIGN IS THRESHOLD-DEPENDENT AND NO SHAPE IS "
                       "CLAIMED (@lacan [5500]; my own 'monotonic crossover' at "
                       "[5498].3 is withdrawn -- it was never monotone and it "
                       "relocates when the inclusion cut moves). Nothing to "
                       "purchase absent a new design. Also withdrawn: the "
                       "[5490] floor reading (checked frequency, the proxy, not "
                       "the gate) and the [5494] composition argument (right "
                       "gate, wrong population -- 57% of its rows had zero "
                       "movement events).",
        }
