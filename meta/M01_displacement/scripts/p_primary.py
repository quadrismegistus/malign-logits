...
SHAPE_PLAIN = ("max_tokens", "model", "prompt", "schema", "system_prompt",
               "temperature")
SHAPE_THINKING = SHAPE_PLAIN + ("thinking",)
DECLARED_SHAPES = (SHAPE_PLAIN, SHAPE_THINKING)

#: **RH's CATCH, AND IT CORRECTS AN ERROR IN THE FIRST CUT OF THIS RULE.**
#: An earlier version dispositioned on SHAPE ALONE and called the plain shape
#: "pre-patch" — letting the shape stand in for the hazard. It does not.
#:
#: [4210]'s hazard is a THINKING-ON entry colliding with a THINKING-OFF call
#: under one key. **That requires the provider to HAVE a thinking mode.** At
#: the pinned checkout `providers.py` sends thinking to ANTHROPIC and to the
#: OpenAI-COMPATIBLE path (DeepSeek) and never to OpenAI proper — so for an
#: OpenAI model no thinking-on variant can exist and no colliding entry can
#: ever have been written.
#:
#: **THE EVIDENCE, from this run's own writes:** all 4,442 gpt-4o-mini entries
#: were written 2026-08-04 14:00-15:00 — P's own window — and NOT ONE carries
#: `thinking`, while the same run, in the same code path, wrote
#: thinking-bearing keys for the other two arms. The plain shape is not old
#: for that model; **it is what an OpenAI key permanently looks like.**
THINKING_CAPABLE = {"anthropic", "deepseek"}
...
def shape_disposition(shape, model):
    """FORCE only where a STALE entry could be REACHABLE. Three cases."""
    if shape == SHAPE_THINKING:
        #: a pre-patch (thinking-less) entry cannot occupy this key.
        return "WARM"
    if _provider(model) not in THINKING_CAPABLE:
        #: **the case the first cut got wrong.** No thinking-on variant can
        #: exist for this provider, so nothing can collide under this key.
        return "WARM"
    #: plain shape on a thinking-capable provider — a genuine pre-patch entry,
    #: reachable only if the call also omits thinking. [4210] STANDS.
    return "FORCE"
    ...
    **RUN 3's NON-THINKING SHAPE IS UNNECESSARY-NOT-EXEMPTED.** [4355]
    ...
    for k in t.stash.keys():
        if not (isinstance(k, dict) and k.get("model") == model):
            continue
        #: **[4422]: COUNT AND PARSE EVERYTHING; BRANCH ON SHAPE ONLY FOR
        #: WHAT TO FORCE.** An earlier cut skipped WARM-shape keys before
        #: this point, which was BEHAVIOURALLY safe — an unparseable WARM key
        #: falls out of `have`, is not forced, and is served warm, which is
        #: what the WARM branch wants anyway — but it pointed [4232]'s
        #: tripwire at the wrong half of the stash:
        #:     FORCE-shape entries are the ones OVERWRITTEN by fresh calls
        #:     **WARM-shape entries are the ones ACTUALLY SERVED**
        #: and the guard was validating the discarded half. `gate_stash_shapes`
        #: does not cover the gap: it compares the FIELD SET, so a change to
        #: the `prompt` field's internal FORMAT keeps the same shape and walks
        #: past it. The two guards catch different things.
        seen += 1
        ...
        if pr and ab:
            parsed += 1
            triple = (re.sub(r"\s*_+\s*$", "", pr.group(1)).strip(),
                      ab.group(1), ab.group(2))
        #: the per-shape rule, [4413]. Only the FORCING decision branches.
        if shape_disposition(tuple(sorted(k.keys())), model) == "WARM":
            warm_shape += 1
            continue
        if triple:
            have.add(triple)
    ...
    if warm_shape:
        print("  per-shape [4413]  %-28s %d entries on the WARM shape "
              "(thinking in key) excluded from forcing; %d keys enumerated "
              "and parsed in total" % (model, warm_shape, seen), flush=True)
...
def gate_response_model():
    ...
    #: **`largeliterarymodels`, NOT `_pr`.** `_pr` is imported LOWER IN THIS
    #: FUNCTION and is unbound here — an UnboundLocalError that killed the
    #: first firing of this gate. **And nothing caught it, for the same
    #: reason as the original crash: `--dry-run` RETURNS BEFORE
    #: `gate_response_model()` runs, so this code path has no dry coverage
    #: at all.** [4357]'s rule was "the dry run must REACH THE WRITER"; the
    #: gates AFTER the dry-run's return are the same blind spot one step
    #: earlier.
    _tagged = subprocess.run(
        ["git", "-C",
         os.path.dirname(os.path.dirname(largeliterarymodels.__file__)),
...
def main():
    ...
    #: **AND THE CAPABILITY DECLARATION IS GATED TOO, not trusted.** The
    #: disposition now rests on THINKING_CAPABLE; if that list is wrong the
    #: rule silently forces or silently warms the wrong arm. The stash says
    #: which providers this run actually sends thinking to: a capable
    #: provider MUST have written thinking-bearing keys, a non-capable one
    #: MUST NOT have. Either mismatch refuses.
    per_model = collections.defaultdict(set)
    from malign_logits.tasks.code_displacement_relation import DisplacementRelationTask as _T
    for _k in _T().stash.keys():
        if isinstance(_k, dict) and _k.get("model") in ROSTER:
            per_model[_k["model"]].add(tuple(sorted(_k.keys())))
    for _m in ROSTER:
        _has = SHAPE_THINKING in per_model.get(_m, set())
        _declared = _provider(_m) in THINKING_CAPABLE
        if _has != _declared:
            raise SystemExit(
                "REFUSING: THINKING_CAPABLE says %s is %s for %s, and the "
                "stash says otherwise — it holds %s thinking-bearing keys for "
                "that model. The per-shape disposition rests on this "
                "declaration; a wrong entry silently forces or silently warms "
                "an entire arm."
                % (_provider(_m), "capable" if _declared else "NOT capable",
                   _m, "some" if _has else "NO"))
    print("         capability declaration agrees with the stash for all "
          "%d roster models" % len(ROSTER), flush=True)
    ...
    for sh, n in shapes.most_common():
        print("         %6d keys : %s" % (n, ", ".join(sh)), flush=True)
