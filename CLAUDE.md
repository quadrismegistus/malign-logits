...
**Liminal content displaces more than explicit — but it is substantially an entropy effect, and "boundary" overstates it.** CORRECTED 2026-07-26, **n corrected 2026-07-27**. The battery holds 9 families but only **8 distinct base→superego pairs**: `tulu` and `tulu-no-safety` share the same base *and* the same superego, differing only at the ego/SFT checkpoint, so every base→superego metric is identical between them to 0.0e+00 across all 47 prompts. For this contrast they are one measurement counted twice. **The reason for the shared superego, corrected 2026-08-10 (docket [5260].3): allenai never released a DPO built on any ablated SFT — the full 107-row Tulu listing was checked, and the paper releases base/SFT/DPO for the full mix only. So `tulu-no-safety` pairs the ablated SFT with the full-mix DPO, a DPO never built on that SFT. The identity is a release fact, not evidence that the safety ablation makes no difference downstream.** Paired, **distinct base→superego pair as unit, n=8**:

- **liminal − explicit JS = +0.0297**, t=+3.18, p=0.0155, 95% t-CI [+0.0076, +0.0518] (bootstrap [+0.0127, +0.0471]), **8/8 positive**. Direction is solid.
- **AND IT SURVIVES THE LINEAGE, checked 2026-08-01.** n=8 collapses tulu/tulu-no-safety on an *exact-duplicate* argument, which is the narrowest possible grounds. **llama, tulu and tulu-no-safety all sit on `meta-llama/Llama-3.1-8B` — three alignment recipes on ONE pretraining run**, so by the campaign's own rule the independent unit is the lineage and n=8 is **7**. At n=7: **+0.0311, t=+2.97, p=0.0249, [+0.0055, +0.0567], 7/7 positive.** liminal−neutral stays null (p=0.52, 5/7). Nothing moves. This arm was added because the identical error class cost F40 its headline interval the same afternoon (39 base strings resampled as 39 lineages; they are 34) — **a claim that survives the check is stronger for having been asked, and this one does.**
- **liminal − neutral JS = +0.0098**, t=+0.68, p=0.52, 95% t-CI [−0.0244, +0.0439], 5/8. **Not significant.** Liminal is indistinguishable from neutral, so there is no boundary *peak* — explicit sits low and everything else is flat.
- **Entropy mediation: DIRECTION HOLDS. The "~91%" IS WITHDRAWN ON EVIDENCE (2026-07-27).** The liminal−explicit entropy gap reproduces exactly (1.315 nats at n=9, 1.294 at n=8). The slope it multiplies does not. Twenty-four specifications were swept — six methods (pooled OLS, mean and median of per-family OLS, within/family-FE, and the last two on category means) × two entropy columns × two cohorts (n=9 and n=8) — and none returns the booked +0.0187/nat. Twelve of the 24 use `entropy_base`, the natural reading of "JS-vs-entropy"; the rest use `entropy_superego` for completeness. The search was stopped there by declared rule: with enough specification freedom something eventually lands, and a thirteenth reading would be fitting rather than reproducing. **Not a lost data state** — `data/battery_results.csv` last changed at `b727374` (2026-07-26 13:16) and `git diff b727374 HEAD` on it is empty, so the figure was computed on exactly this data. It is an arithmetic or transcription error. Under the reproducible methods the share explained is **67–79% at n=8** (72–84% at n=9). Cite the entropy effect as **substantial**; do not cite 91%, and do not write "almost entirely explained by entropy" — that clause is what turned a partial mediation into a debunking. Re-derive with `scripts/liminal_explicit_recheck.py`.
