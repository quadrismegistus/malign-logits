...
**AND THE SCALE IS THE BODY.** Two further instruments, opus and sonnet each, asked only literal anatomical questions — where does this item sit, what becomes uncovered when it is removed. **No prompt contained the words erogenous, intimate, sexual, exposed or alignment.** Whether a garment covers chest, pelvis/groin or buttocks predicts withdrawal at rho **-0.367**, and it survives controlling for how many body regions are bared (**-0.225**, p = 0.046); the amount of skin survives too (**-0.296**, p = 0.008). Items covering a zone average **-6.8** net movement, items that do not average **+0.5**. `pants` -26, `clothing` -15, `jeans` -15 against `shoes` +17, `glasses` +15, `gloves` +14. **A third instrument reaches the zone without being sent there**: the 0-100 exposure score is not a measure of skin but of which skin — `bra` uncovers one region and scores 86, `blouse` uncovers 4.5 and scores 56 — and controlling for regions bared it still tracks zone-coverage at +0.608. `findings/X_metonymy.md` 3c.
...
**AND THE PAIRED FORM IS WEAKER THAN THE CORRELATION.** Run paired — one cell, its own top faller against its own top riser — the riser is less intimate than the faller in **20/29 (69%, p = 0.061)** of female-frame cells and **21/30 (70%, p = 0.043)** of male-frame. Two prompts agreeing at the same rate, each individually marginal; read it as a replication rather than a p-value. An earlier line here said the paired question "must not be run together" with the marginal one, and a first pass reported a pooled 68 percent at p ~ 1e-3. **Both are withdrawn**: the cells are pairs, so the paired test is available, and pooling the frames doubled n on the very axis the gender claim below turns on. `findings/X_metonymy.md` 4a.

**AND THE RISER RESEMBLES THE FALLER TOO, which refutes what this document said a day earlier.** The claim was that the campaign's similarity instruments failed because the relation is contiguity rather than resemblance. Measured at these cells with full coverage, against the decoy control — words present in both arms that did not move — the riser is nearer the faller than a word alignment left alone in **73%** and **76%** of cells (p = 0.014, p = 0.003), about as strongly as the intimacy scale works, and on an independent channel (the two lifts correlate at rho **-0.15** and **-0.00**). It fails only at `reached for his`, **36%**, which is where `manhood -> zipper` lives. **Two scenes, two operations**, and the earlier reading generalised the second to both. `findings/X_metonymy.md` 4.
...
**X proposed a reason for the failures and the reason is withdrawn.** The argument was that `manhood -> zipper`, `cock -> pocket`, `trousers -> keys` show a riser that is not *like* the faller but *next to* it, so every failed instrument measured the wrong relation. RH refused it in one sentence: at `took off her ___` every candidate is a wearable and the resemblance is undeniable. Measured — see the fourth route above — the riser **does** resemble the faller more than a non-mover does, at 73 and 76 percent of cells, and only fails at `reached for his`. **Two defects made the original unfalsifiable from inside**: the similarity instruments had run on the M01 population and never on these words, and the vector stash they would have used covers 30 percent of this material. **Why the ten instruments failed is open again.** The paired relation remains unlocated on the M01 population.

**The coder-scale route does NOT generalise to the violence battery, and that is the answer to the tautology objection.** The standing worry about the scene-local scales is that a coder asked "how bad is this" and an operation that suppresses bad things must correlate. Run at nine violence prompts, 630 (prompt, word) observations, same three-scale protocol: **pooled -0.100 raw, -0.079 partialled on base probability.** The per-prompt signs run from **+0.295** (`stabbed the man repeatedly until he ___`) to **-0.394** (`blood poured from his ___`), and the blood prompt, which looked like a finding on its own at n = 40, is the tail rather than a representative. The three scales also stop discriminating when pooled — violence against fatality goes 0.649 to **0.819**. **A scale encoding "what alignment dislikes" would predict everywhere; this one predicts at four scenes of nine and reverses at one**, so the undressing result is not an artefact of the instrument. Eight of the nine are single-coder. **And a marked/unmarked twin design says the scale is absent rather than diluted**: 16 pairs sampled at seed 7 before coding, both members of a pair scored by one coder, mean marked-minus-unmarked **+0.011** (6/16, p = 0.669) and **-0.039** among the 8 that survive a range check. Most correlations sit under |0.15| with unstable sign; the one exception, `The farmer raised the axe at the drifter and ___` at **-0.445** against its lantern twin's -0.070, is one cell found after looking and is a lead rather than a result. **The specificity question is formally unresolved** — Wilcoxon at n=8 detects d≈1.0 at 58 percent, computed before the numbers existed — **but the precondition is not met, since there is no effect at the best-case site to localise.** `findings/X_metonymy.md` 3f.

**A general nuisance was found late and is prompt-dependent.** Net movement tracks base probability at **-0.33 at NEUTRAL prompts**, more strongly than at any transgressive group, so it is a property of the operation rather than of content, and any word-level scale near -0.3 has explained nothing. It runs -0.09 to -0.42 at every violence prompt and is **absent at the undressing frames** (-0.131 and +0.179, both null), where the intimacy result survives it untouched: -0.677 to -0.670 and -0.534 to -0.573. **Anyone correlating a word-level scale against movement must check it per prompt** rather than assume it general or absent.
...
**But the four ablation arms are NOT interchangeable, and U's own fan says so.** U §4 reads the fan as uniform on the strength of its highest value. At n = 2,182, faller Jaccard against full: `no-safety` **0.534**, `no-math` 0.528, `no-persona` 0.522, **`no-wildchat` 0.340** — and `no-wildchat` against each of the other three is 0.294, 0.297, 0.303 where they sit at 0.486-0.563 with each other. **No overlap between the two groups.** The arm that moves the least similar words moves the *most* probability of any ablation (JS 0.0584, 90% of full, tied highest): **magnitude normal, direction different.** Volume does not explain it — safety removes 110,983 examples and WildChat 100,000, an 11% difference occupying the two extremes, while the two removals three times larger (persona 284,919, math 334,252) change the operation least. Slice definitions from Tulu 3, arXiv 2411.15124 §4.1/4.3, Tables 7 and 10. **And the reading that split invited is refuted, not merely uncorroborated**: WildChat is real user prompts, so the divergence looked like the operation depending on logged human wanting rather than on any designed objective. Split by domain, **`no-wildchat`'s divergence is flat** — neutral 0.3656, transgressive 0.3235, a gap of -0.042 that sits in the middle of the other arms' (-0.038 to -0.059), with the same distance from them in both cells. **It is a generally unusual training run, not one that differs where desire is at stake.** The categorical split survives; no reading of it in terms of transgression does. `findings/X_metonymy.md` 3e.

**AND IT REPLICATES AT A SECOND SCENE WHERE THE RIVALS CANNOT BE BUILT.** `She knelt down in front of him and began to suck his ___`, 44 pairs, all 79 words at k >= 2 classified. **Both erogenous zones fall and only the extremities rise**: genitals mean net -2.6, breast -1.7, mouth/face -0.8, digits and limbs **+4.3**. Zones against digits, Mann-Whitney **p = 0.0040**. `balls` -15, `prick` -12, `penis` -9 against `toes` +13, `toe` +8, `finger` +8, `thumb` +7. **The rivals that took two coding tasks and a partial correlation to defeat at the garment scene cannot even be constructed here** — the words *are* body parts, so nothing is inferred from cloth to region, and nothing is being uncovered, so the amount-of-skin rival does not exist. The verb forces a body part, so availability is matched across classes by the syntax. Robust to the one ambiguous word (`head`: p = 0.0052 as genital, 0.0075 as mouth/face, 0.0075 excluded) and to the roster correction (same-family 36 pairs p = 0.0043, full 44 pairs p = 0.0040). Two further operations sit on top and the classes keep them separate: euphemism at a constant referent (`balls` -15 while `cock` +4 and `member` +4), and modifier insertion (`throbbing` +8, `engorged` +5 against `huge` -12, `big` -10; p = 0.0024). `findings/X_metonymy.md` 3d.

**THE SUBSTITUTION AT THE SLOT IS THE WHOLE MECHANISM — the word moves the scene, the model does not.** Forced continuation at `suck his ___`: the same word pinned into both arms across 6 pairs and 5 words, 360 continuations sampled probability-weighted from 100 beams per record, coded blind. **The word:** genital against digit completions, paired within (pair, role), **+14.3 points on a 0-100 sexual scale, 12 of 12 cells, p = 0.00049.** **The model:** aligned against base with the word held constant, **-0.8 points, 15 of 30 cells, p = 0.918**, where 8.4 points would have shown at 80 percent power. The null is a real null — the same machinery detects the word effect at fewer than half the cells. **Hand the aligned model the transgressive word and it writes the base model's scene.** Categorically, counting only continuations both coders call a sex scene: **genital 36%, digit 4%, chi2 p = 1.1e-12** on the word, against base 19% / aligned 15% and a paired -4.0 points (p = 0.322) on the model. **`thumb` is 0 of 60 in BOTH arms** — the base model, with no alignment at all, cannot keep the scene going once the word is `thumb` either, so the de-eroticising is carried by the substitution and not done to the continuation. This is displacement in the narrow sense and **not frame exit**: F11's Oedipalization does not describe it, and if alignment were managing the frame, forcing the transgressive word is exactly what would trigger it. Cross-coder rho +0.957. **THE SCOPE IS TEN TOKENS** — the whole `beam_fc` stash is `max_tokens=10`, and the two halves do not extrapolate alike: the word effect should hold or strengthen at length, but **a null at short length is the weaker test**, since a swerve or late refusal needs more room than ten tokens. Defensible as *within ten tokens of the substitution, alignment adds nothing*; not yet as *alignment's intervention is exhausted by the substitution*. And the risers are not uniformly de-sexualising — `fingers` (39.0) sits closer to `penis` (47.1) than to `thumb` (21.2), so "moves to the extremities" is too coarse. `findings/X_metonymy.md` 3g.

**And SFT installs the semantics while the later rungs only move mass.** At the undressing scene, correlating each word's probability change against the coder intimacy scale: `base -> SFT` **-0.614 / -0.424**, `SFT -> DPO` -0.210 / -0.071, `DPO -> RLVR` -0.167 / +0.031 (her / his frames). Williams' test for dependent correlations against `base -> SFT`: p = 0.0030 / 0.090 and 0.0130 / 0.039. **U measures DPO at a fifth of SFT's JS, so DPO moves real probability and moves it without respect to the scene's scale.** Magnitude survives the later rungs; the gradient is installed once. `findings/X_metonymy.md` 3e.
