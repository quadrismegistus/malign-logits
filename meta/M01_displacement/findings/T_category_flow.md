...
**AMENDMENT CORRECTED 2026-08-12, same day (pen; the morning version of this note overclaimed and is superseded).** Two true facts and one wrong inference passed through this note before noon. TRUE: (1) the marginal's deltas sum to zero within every (stratum, labeling) by construction (verified 8.8e-15, three seats) — so in the UNFILTERED population the magnitude ratio EQUALS the count ratio identically (2.05x = 2.05x on this artifact) and the two are one fact, not two. (2) The slice of record for this section's numbers: stratum=ALL, seven category labelings (TOKEN excluded), Bonferroni survivors — 206/36, +0.00334/-0.01267, 3.79x; other slices give 1.79-5.50x, so no magnitude travels without its slice named. WRONG, and withdrawn: the morning claim that the identity retires the 3.8x. **This section's numbers are Bonferroni SURVIVORS — categories whose delta is consistent across the 43 edges (`test_marginal`, one-sample t per category over edges) — and the zero-sum does NOT bind that subset** (survivor sums +0.688/-0.456; pure forcing would give 5.72x against the observed 3.79x). The quoted ratio is a fact about which categories move consistently, not arithmetic. The section's own paragraph below ("The count and the magnitude do not have the same standing") already recorded the needed discipline in July; the morning's retirement went past it and is struck.
...
## 16. Counting fields instead of labellings
...
## Data and code
    ...
    dedup         meta/M01_displacement/scripts/s_cluster_dedup.py
                  Jaccard over word sets; results/s_cluster_dedup.csv;
                  s_jaccard_clusters.csv; finding 16
