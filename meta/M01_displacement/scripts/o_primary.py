...
def qualifying_edges(pairs):
    ...
    #: ASSERTION (3), [4127]/[4129]. COMPETENCE_BASIS is prose; THIS is the
    #: behaviour. Ruling 2 says all 301 zh prompts, and a constant spelling
    #: "all-301-zh" would still read "all-301-zh" if a slice appeared here.
    if len(zh) != 301:
        raise SystemExit("REFUSING: ruling 2 measures competence on ALL 301 zh "
                         "prompts; this call has %d." % len(zh))
    shares, missing = {}, []
...
def main():
    ...
    results, readings = {}, {}
    #: §O1.3's own analysed counts. ASSERTION (1), [4127]/[4129]: H1_CELL_SET is
    #: prose and this is the QUANTITY IT GOVERNS -- ruling 1's outcome was the
    #: only registered figure in this file that nothing armed.
    H1_N = {"en": 2642, "zh": 2604}
    for name, field, predict, gloss in H:
        en = run_arm(rows["en"], field, fam_base, predict)
        zh = run_arm(rows["zh"], field, fam_base, predict)
        if name == "H1":
            for arm, a in (("en", en), ("zh", zh)):
                if a["n_cells"] != H1_N[arm]:
                    raise SystemExit(
                        "REFUSING: H1 ran on %d %s cells; ruling 1 (a) and "
                        "§O1.3 put it on ALL %d ANALYSED cells. The constant "
                        "still spells 'analysed'; the behaviour moved."
                        % (a["n_cells"], arm, H1_N[arm]))
        #: ASSERTION (2), [4127]/[4129]: §O3 and §O1.3 both register NINE
        #: clusters. A family whose z is None leaves `by_base` silently, so
        #: this is the only place a dropped cluster becomes visible.
        for arm, a in (("en", en), ("zh", zh)):
            if a["n_clusters"] != 9:
                raise SystemExit(
                    "REFUSING: %s %s combined %d clusters; §O3 combines NINE "
                    "base checkpoints with equal weight. A cluster left the "
                    "Stouffer without being counted."
                    % (name, arm, a["n_clusters"]))
    ...
    for arm in ("en", "zh"):
        b = bias[arm]
        #: [4127] F4: both are None when NO cell has a defined push, and this
        #: print sits BEFORE the artifact write in a once-only run. Its two
        #: neighbouring blocks guard exactly this; this one did not.
        fmt = lambda v: ("%.3e" % v) if v is not None else "n/a"
        print("  %s  push median %s  max %s  (undefined %d)"
              % (arm, fmt(b["median"]), fmt(b["max"]), b["n_undefined"]),
              flush=True)

    #: THE RIDER, [4128] drafted / [4130] approved VERBATIM, binding from
    #: before the numbers existed. It changes no test -- every registered
    #: quantity is a sign test and signs survive any positive affine map --
    #: and it exists because this artifact stores per-cell A for BOTH arms in
    #: one file, so a reader CAN difference them. A limit stated after a
    #: result is a limit chosen by the result.
    RIDER = ("A_absvalence magnitudes are NOT comparable across arms: zh "
             "valence sd 0.9697 vs en 1.2753 (ratio 1.32), so identical raw "
             "displacement yields a 32% larger zh statistic. SIGNS are "
             "comparable; the registered tests are sign tests. A_arousal "
             "magnitudes ARE comparable (sd ratio 1.01).")
    payload = {
        "_rider_cross_arm_magnitudes": RIDER,
