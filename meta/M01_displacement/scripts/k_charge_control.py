...
THREE MODELS, REPORTED TOGETHER BECAUSE THEY ANSWER DIFFERENT QUESTIONS:
  ...
  INTERACTION   the transgressiveness effect computed separately among
                ...
                median, and the difference tested
...
def main(lang="en"):
    ...
    for i,(p,v) in enumerate(byp.items()):
        ...
        for tag,R in (("real",rate),("shuf",SH)):
            ...
            med=np.median(ch); hi=np.where(ch>med)[0]; lo=np.where(ch<=med)[0]
            if len(hi)>=MIN_HALF and len(lo)>=MIN_HALF:
                for nm,ix in (("hi-charge",hi),("lo-charge",lo)):
                    ...
                    xx=A.resid(A.ranks([R[w[j]]["transgressiveness"] for j in ix]),ll)
    ...
    n=min(len(hi),len(lo)); d=hi[:n]-lo[:n]
    z=d.mean()/(d.std(ddof=1)/np.sqrt(n))
    print("\n  INTERACTION  trns@hi minus trns@lo:  mean %+.4f   z %+.1f   n=%d"%(d.mean(),z,n))
