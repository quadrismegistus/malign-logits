...
**STATUS, FROZEN 2026-08-05 UTC on RH's word ([4487], verbatim: *"Freeze and run Q. Thanks all!"*), signed against the list at [4484].** As of that date and pasted from the commands that emitted them: **no `result_q_*.json` existed at any seat; no seat had read a hypothesis quantity on any partition of N's population; no producer had been written.** The freeze is on content that RH signed at `sha256:1611c0de342e09c7`; **this header is the only substantive change made after that signature**, and the diff between the signed bytes and the frozen bytes is the header alone plus the pass enumeration corrected below.

**WRITTEN IN THE PAST TENSE ON PURPOSE, AND A DATE IS NOT ENOUGH.** *"DRAFT, not frozen"* carried a date and was still a PRESENT-TENSE claim: **true until the freeze commit and false forever after, with nothing to rewrite it.** This campaign has met status headers that outlived their truth — D3b's *"NO D3b QUANTITY HAS BEEN COMPUTED"*, still in place after both stages had run — and corrected N's and O's for exactly this reason. **A dated statement of what was ABSENT on a named date stays testimony; a dated statement of what IS expires.**

**FIVE PRECONDITION PASSES HAVE RUN, AND NONE IS A PRODUCER.** All are precondition instruments that emit counts or a dispersion and cannot emit a hypothesis quantity. **Every one is named with the hash function that pins it — `sha256`, first 16 hex — because this registration requires its own counts to be reproducible from a NAMED instrument rather than a described one (§Q1.3), and for one round it did not hold itself to that:**

    `p_yield_pass.py`            @ sha256 `e7cf89598d9e095d`  — §Q1.3's yields, counts only
    `q_h1_sd_pass.py`            @ sha256 `eef2f6047749fdda`  — H1's dispersion, first cut
    `q_dispersion_pass.py`       @ sha256 `ac9ec59f56ae8ff2`  — **§Q4's H1-reproduce + H5 + H6 dispersions**
    `q_h6_denominator_pass.py`   @ sha256 `90a81833eaafabfa`  — H6's denominator, counts only
    `q_u2_weighting.py`          @ sha256 `ce68920458d2273d`  — **U2's price: H2's SE, counts only**

**AND THE FIVE DO NOT CARRY THE SAME ATTESTATION, SO THE ESCROW FILENAMES DO NOT CLAIM THEY DO.** Three were cleared at a second seat before running and are escrowed `CLEARED-`: `q_h1_sd_pass`, `q_dispersion_pass`, `q_h6_denominator_pass` (with `q_dispersion_pass`'s REFUSED first cut kept at `CLEARED-1aa78d9e.py`). **Two were KNOWN-ANSWERED against a published figure and never separately cleared** — `p_yield_pass` and `q_u2_weighting` — and are escrowed `DEPOSITED-`, a label that claims only what happened. **A filename asserting a clearance that did not occur is a fabricated attestation, and the cheapest place to make one is a copy command at a freeze.**
**THE HEADER SAID "TWO" UNTIL THE FREEZE GATE, AND THREE OF THE FIVE WERE ADDED THERE.** `q_dispersion_pass.py` produced **two of the six MDEs in §Q4's table** — H5's and H6's — and §Q4 described it (*"one construction-blind pass emits three `(sd, k)` pairs"*) without ever naming it. **A described instrument is not a pinned one**, which is the distinction this document draws at §Q1.3 and then failed at three hundred lines later. Its escrows were in custody throughout — `scripts/audited/q_dispersion_pass.CLEARED-ac9ec59f.py`, and the REFUSED first cut at `CLEARED-1aa78d9e.py` — **so the gap was in the text and never in the record.**
...
## §Q2 THE MEASURES — one pass, four quantities per cell
    ...
    tail_excess      the residual bin's own excess. NEGATIVE = the tail gave
                     ...
                     **AND THE TWO ARMS ARE NOT INTERCHANGEABLE FOR A
                     DISPERSION, WHICH IS STATED HERE AND NOT ONLY IN §Q4.**
                     `tail_excess_corrected = tail_excess_raw + push`,
                     where `push = A.residual * dR / S` — **a definition
                     that lives in N's PRODUCER and NOT in the `movement.py`
                     module §Q0 pins.** The two arms agree to six decimals
                     at the POOLED level, **which is a statement about a
                     MEAN and licenses nothing about a SPREAD**: two arms
                     can share a pooled mean to any precision and differ in
                     dispersion. **A pass that recomputed the raw arm and
                     compared it to a corrected-arm constant refused on
                     2026-08-04, which is what this paragraph exists to
                     prevent happening twice.**
                     **CONSEQUENCE FOR ANY PRODUCER: the corrected arm is
                     READ FROM N's ARTIFACT and never re-implemented.**
                     Copying `push` into Q would put a second
                     implementation of another registration's definition
                     into the campaign, and Q could never be its authority
                     — the same objection that removed G and D2 from §Q6.
...
**H1 — SUBSTITUTION AT SITES.** `tail_excess` differs between the marked and unmarked members of a minimal pair.
    ...
    TEST       paired, sign-flip null, TWO-SIDED, `p < 0.0125`
    MDE        **0.00189** — see §Q4. The best-provenanced row in the
...
**H2 — SUBSTITUTION AT SITES, IN THE GENERAL CORPUS.** `tail_excess` differs between `nonpair_transgressive` and `nonpair_neutral`.
    ...
    TEST       paired over clusters, TWO-SIDED, `p < 0.0125`
    FLOOR      a cluster enters only with **>= 10 ANALYSED cells on BOTH
               ...
               **THE VALUE IS ARBITRARY; THE EXISTENCE OF THE FLOOR IS NOT,
               ...
               and 1 cells does the rest:
                   ...
                   floor >= 10   k=33   SE 0.001305   80% MDE **0.00436**
                   no floor      k=34   SE 0.002317   80% MDE **0.00774**
                                                      — **1.78x worse**

               **Both figures are at the registered two-sided alpha 0.0125**
               (multiplier 3.3393, the FOUR-way split). An earlier draft quoted 0.00366 and
...
**H4 — MAGNITUDE AT SITES, IN THE GENERAL CORPUS.** `departed` differs between `nonpair_transgressive` and `nonpair_neutral`.
    ...
    TEST       paired over clusters, TWO-SIDED, `p < 0.0125`
    MDE        **0.00571** — see §Q4 for its provenance, which is not this
               population's
...
**H5 — MAGNITUDE AT SITES, IN THE PAIR FRAME.** `departed` differs between the marked and unmarked members of a minimal pair.

    UNIT       the PAIR, **k = 684**
    THE KEY    H1's, unchanged: an edge is `(base, aligned)`, family labels
               collapse, the shared transition contributes ONCE
    DENOMINATOR **24,606 both-sides keys — H1's exactly.** §Q1.2 clause 7
               puts `departed` on the ANALYSED denominator, the same one
               `tail_excess` uses, because `departed` is **zero by
               construction** where no faller cleared the rule. **No new
               counting was required and none was done.**
    TEST       paired, sign-flip null, TWO-SIDED, `p < 0.0125`
    MDE        **see §Q4 — PENDING the dispersion pass.**

**H5 IS WHAT §Q3's FIRST CUT DECLINED, AND DECLINING IT WAS THE ERROR.** That cut said *"the pair-frame version is NOT a hypothesis here — it would re-derive G"*. **G runs on 34 clusters against Q's 33 and D2 on 632 pairs against Q's 684: they are not on Q's scale, which is the entire reason this registration exists.** Q pushed a row of its own grid out of scope for the very fact that makes the bridge necessary. **Without H5 the magnitude row has no twin cell and no ratio.**

**H6 — THE NORM SIGNATURE IN THE PAIR FRAME, ESTIMATED AND NOT TESTED.** `A_|valence|` on the marked-vs-unmarked contrast.

    UNIT       the PAIR, **k = 626** of 684 stems
    DENOMINATOR **15,152 both-sides A-CELL keys**, measured by
               `q_h6_denominator_pass.py` @ `sha256 90a81833eaafabfa`,
               counts only. `A` runs on A-cells (§Q1.2 clause 6), so a pair
               enters only where BOTH members qualify.
    TEST       **NONE. No alpha is consumed and no verdict language
               attaches.** Reported as a point estimate with an interval
               and its stated MDE, exactly as H3 is.
    MDE        **0.01773** at alpha 0.05 (the conventional statement for an
               arm consuming none), from `sd_d = 0.158320`, k = 626.

**H6 WAS DRAFTED AS A TESTED ARM AND DEMOTED ON ITS OWN MEASURED DISPERSION, WHICH IS THE ONLY HONEST ORDER FOR THAT DECISION.** The blind pass returned `sd_d = 0.158320`, giving an MDE at the five-way split of **0.02162 — 1.42x D2's measured valence effect of 0.015246, the very effect that justified admitting the arm.**

    **POWER FOR THAT EFFECT: 43% at alpha 0.01, 67% as an estimate at
    0.05.** Convention is 80%.
    **AND THE RATIO MUST TRAVEL WITH ITS ALPHA, BECAUSE H6 HAS TWO OF
    EACH.** At the five-way alpha 0.01 the arm was decided on, MDE
    **0.02162** = **1.42x** D2's +0.015246. **At the alpha 0.05 it now
    carries as an estimate, MDE 0.01773 = 1.16x.** Both are exact; they
    are one arm at two alphas. **The demotion stands at either — 1.16 is
    still above 1, and the arm cannot resolve the effect that justified
    admitting it — but it is short by 16%, not by 42%, at the parameters
    this registration actually registers.**
    **This is NOT "cannot detect."** D2 reached p = 0.0076 on an effect
    BELOW its own stated `raw_mde` of 0.017376. **An underpowered arm can
    still fire; it fires unreliably, and its NULL means almost nothing** —
    the exact failure §Q4 records for H3 and for Registration C's control
    arm.

**AND THE ARGUMENT THAT ADMITTED H6 WAS HALF RIGHT, WHICH IS RECORDED RATHER THAN QUIETLY DROPPED.** H3 was declared estimate-only on an MDE computed for the **CLUSTER unit at k = 32**, and the pair unit's MDE had never been computed — **that criticism was correct and it is why this arm was built.** But the pair unit's DISPERSION (0.158320) is large enough that k = 626 does not rescue it. **The unit was wrong and the conclusion survived the correction.**

**SO H6 DOES NOT SPLIT H3 AFTER ALL; IT JOINS IT.** H3 declared the norm signature estimate-only on **MDE 0.03693 — computed for the CLUSTER unit at k = 32.** The pair unit has **k = 626** and its MDE was never computed. **A whole arm was ruled unbridgeable on a power figure belonging to the other half of the bridge** — the defect class of a power figure stated for a quantity other than the one being decided. **The pair contrast becomes TESTED; the corpus contrast remains ESTIMATED, each with its own MDE.**

    **AND THE RETENTION IS MEASURED RATHER THAN ASSUMED:** 15,152 of
    24,606 both-sides keys survive the A-cell requirement — **61.6%,
    against 48.3% under independence — so A-cell survival is POSITIVELY
    CORRELATED within a pair.** At the stem level **626 of 684 survive
    (91.5%)**, because a stem needs one surviving both-sides key and not
    all of them. **H6 costs 4.5% on the MDE against H1 for the same
    dispersion** (`sqrt(684/626) = 1.045`).
...
**ALPHA 0.05, SPLIT FOUR WAYS — `p < 0.0125` EACH — ACROSS Q's FOUR TESTED ARMS (H1, H2, H4 and H5).** H3 and H6 are ESTIMATED, consume no alpha, and carry no verdict language. D2's split form, extended twice: once when H4 entered on RH's word, and again when H5 and H6 entered to complete the 3x2 grid RH's occasioning question asks for.

    **THE COST OF THE TWO NEW ARMS, PRICED BEFORE THEY WERE ADMITTED** —
    the rule H4's own entry established:
      3 arms  alpha 0.016667  z 2.3940  multiplier 3.2356
      **4 arms  alpha 0.012500  z 2.4977  multiplier 3.3393 = 2.4977 + 0.8416**
      **every existing MDE grows 3.2%:**
        H1  0.00183 -> **0.00189**   (2.48% -> **2.55%** of N's −0.0738)
        H2  0.00422 -> **0.00436**
        H4  0.00554 -> **0.00571**
    **Trivial BY A STATED CRITERION, which is the only way an adjective
    about a quantity may stand: no arm's MDE crosses an effect size this
    campaign has measured.** The two arms that HAVE such a benchmark —
    H3 and H6, against C's +0.025 and D2's +0.01525 — are untouched by
    this split because they consume no alpha, and the three that move
    have no registered effect size to cross. **That second clause is a
    gap, not a clearance, and §Q4 now says so.** Stated as a measurement
    rather than an assurance.

    **A FIVE-WAY SPLIT WAS DRAFTED AND IS NOT WHAT THIS REGISTRATION USES.**
    H5 and H6 were admitted together at `p < 0.01` (multiplier 3.4175);
    **H6's measured dispersion then demoted it to an estimate, and the
    alpha returned to four ways.** The intermediate figures appear nowhere
    else in this document — **an alpha that moved twice must leave one
    threshold, not three.**

    **THE EARLIER ROUND, KEPT AS RECORD AND SUPERSEDED BY THE FIGURES
    ABOVE.** When H4 alone was admitted the split went two ways to three,
    and its cost was priced the same way. **These numbers are the THREE-arm
    split and none of them is current:**
      H2   0.025 -> 0.0167   80% MDE 0.00402 -> 0.00422, +4.9% —
           5.45% -> 5.72% of N's level
      H1   0.00183 at the three-way split
      H4   0.00554 at the three-way split, against a `departed` scale
           whose within-cluster SD is 0.0789
    **Retained because the rule it demonstrates — price an arm's cost to
    the others BEFORE admitting it — is the rule H5 and H6 were admitted
    under, and a registration that shows the rule working twice is worth
    more than one that shows the latest numbers only.**
...
**COMPUTED FOR THE ESTIMATOR §Q3 REGISTERS — the CLUSTER, paired within cluster (the PAIR for H1) — and at 80% power**, which is this campaign's convention (D2 §A7: *"80% power, simulation at realized pair-count and variance, RAW scale"*).
    ...
    **TESTED — alpha 0.05 split four ways, multiplier 3.3393**
    H1  tail_excess, k=684       0.000565   **0.00189**   0.0125  **N's own**, spent, BLIND
    H2  tail_excess, k=33        0.001305   **0.00436**   0.0125  **N's own**, spent
    H4  departed, k=33           0.001711   **0.00571**   0.0125  **L's FOUND PROSE**, borrowed
    **H5  departed, k=684        0.000852   0.00284**     0.0125  **N's own**, spent, BLIND
    **ESTIMATED — no alpha consumed, no verdict language**
    **H6  A_|valence|, k=626     0.006328   0.01773**     0.05    **N's own**, spent, BLIND
    H3  A_|valence|, k=32        0.013181   **0.03693**   0.05    **O's**, borrowed
                                                                  (CORPUS arm)

    **H5's AND H6's DISPERSIONS WERE MEASURED BLIND AND ARE NOW IN**, by
    `q_dispersion_pass.py` @ **sha256 `ac9ec59f56ae8ff2`** (escrowed
    `scripts/audited/q_dispersion_pass.CLEARED-ac9ec59f.py`; its refused
    first cut at `CLEARED-1aa78d9e.py`). Both are PAIRED, so each needs `sd(d_i)` — and the SD of
    the paired differences and their MEAN come from one subtraction, with
    the mean being the test statistic. **No seat that will read them may
    compute them.** One construction-blind pass emits three `(sd, k)` pairs
    — H1's, H5's and H6's — through identical code, and **its H1 output
    must reproduce the published `0.014765 / 684` or it refuses.** A blind
    instrument that re-derives a known number before emitting two unknown
    ones is self-verifying in the only way a blind instrument can be.
      **MDE(H5) = 3.3393 x 0.022282 / sqrt(684) = 0.00284**
        **AND THE FIFTH DECIMAL IS NOT DETERMINED BY THE RECORD.** The
        blind egress is a `%.6f` print, so `sd_d` is known only within
        [0.0222815, 0.0222825), which puts this MDE in
        [0.0028449, 0.0028451] — an interval that STRADDLES the 5-dp
        rounding boundary. 0.00284 is what the stated inputs give and is
        what is registered; **nothing in Q may turn on that digit**, and a
        comparison that needs H5 should quote it at 4 dp as 0.0028.
        Widening the egress to fix this would cost more than the digit is
        worth — an extra decimal of a dispersion is a step toward the
        effect, and the digit buys nothing.
      **MDE(H6) = 2.8016 x 0.158320 / sqrt(626) = 0.01773**  (alpha 0.05,
        the conventional statement for an arm consuming none)
    **THE PASS REFUSED ONCE BEFORE IT PRODUCED THESE.** Its first cut
    recomputed `tail_excess` from the pinned machinery — the RAW arm — and
    compared it to the published constant, which is a dispersion of
    CORRECTED differences. **The known answer caught it and neither new
    number was read.** The re-cut reads N's artifact for H1 and recomputes
    only what the machinery defines; **H1 then reproduced `0.014765 / 684`
    exactly**, and the two new numbers were emitted behind that gate.

**EVERY ROW NAMES ITS ALPHA AS WELL AS ITS DISPERSION, BECAUSE THE TABLE MIXES BOTH.** H1, H2, H4 and H5 are at the registered two-sided **0.0125**, multiplier **3.3393** = 2.4977 + 0.8416, the exact four-way split 0.05/4. (An earlier draft of this sentence carried the multiplier's NEW value beside its OLD decomposition, `2.3940 + 0.8416`, which sums to the superseded 3.2356 — **a line that names a constant correctly and then hands the reader the ingredients of the previous one**, and the only defect in the [4462] round that a sweep for the changed constant would have landed on and read past.) **H3 is at two-sided 0.05** — the conventional statement for an arm that consumes no alpha and runs no test; at 0.01 it would be **0.04505**, and H3's conclusion is the same at either, which is why the looser figure is quoted and labelled rather than hidden.

**AND FOUR ROWS NAME NO EFFECT SIZE AT ALL, WHICH IS WHY NO ROW CARRIES A VERDICT ON WHETHER IT IS "ENOUGH".** An MDE is half of a power statement; the other half is the effect the arm must detect. **This campaign has measured exactly two effect sizes — C's general valence +0.025 and D2's +0.01525 — and both are on the `A_|valence|` scale.** They license the two norms rows: H3's 0.03693 exceeds both, H6's 0.01773 exceeds D2's. **The four `tail_excess` and `departed` rows have no measured effect size to be read against, so their MDEs state RESOLUTION and nothing further.** H1's 2.55% and H2's 5.90% are fractions of a LEVEL, H4's 7.2% and H5's of a DISPERSION, and **neither fraction is a power verdict.** **This paragraph exists because an adjective did the work instead**: "H2 is powered by a wide margin" stood at §Q4's prose through three different values of the ratio beneath it, naming no threshold at which it would fail, and is withdrawn at [4465]'s finding. **A registration that cannot say how large an effect it expects should say that, not say the arm is powered.**
...
**THE ROW FILLED FROM A FORMULA THAT WAS IN THIS TEXT BEFORE `sd_d` EXISTED — THE NUMBER FILLED A SLOT RATHER THAN CHOOSING ONE:**
    ...
    **MDE(H1) = 3.3393 x sd_d / sqrt(k)**,  k = 684 stems

    **sd_d = 0.014765**,  **k = 684**   — the pass's entire output
    **MDE(H1) = 3.3393 x 0.014765 / sqrt(684) = 0.00189**, checked at a
    second seat. **Against N's pooled level of −0.0738 that is 2.55%.**

    **H1 IS THE BEST-POWERED OF THE TWO `tail_excess` ARMS AND HAS THE
    BEST PROVENANCE IN THE TABLE.** The scope is deliberate: it read
    "the best-powered arm in this registration", which **compares across
    quantities whose yardsticks differ** — H1 and H2 are quoted against a
    LEVEL (N's −0.0738), H4 and H5 against a DISPERSION (L's 0.0789), and
    **a ratio to a mean and a ratio to an SD are not the same measure.**
    H1 is lowest on both readings, but the claim as written asserted a
    comparison the table cannot make.** Its MDE is **0.43 of H2's** — **a
    little over two-fifths and NOT "a third"**, a ratio
    ...
    **3.3393 = 2.4977 + 0.8416** — the TWO-SIDED z at **alpha 0.05/4**
    (§Q3.1's FOUR-way split, `p < 0.0125` exactly) plus the 80%-power z.
    **The multiplier moved twice: 3.2356 at three arms, 3.4175 when H5 and
    H6 were both admitted, and 3.3393 when H6's measured dispersion
    demoted it. Every MDE above is stated at 3.3393 and no intermediate
    figure survives anywhere in this document.** **The two constants it is not, and both have been used
...
**H2's MDE IS 0.00436, WHICH IS 5.90% OF N's POOLED `tail_excess` LEVEL (−0.0738) — AND THAT IS A STATEMENT ABOUT RESOLUTION, NOT ABOUT POWER.** **"H2 IS POWERED BY A WIDE MARGIN" STOOD HERE AND IS WITHDRAWN.** Power is relative to the effect an arm must detect, and **Q REGISTERS NO EXPECTED EFFECT SIZE FOR THE TRANSGRESSIVE-vs-NEUTRAL `tail_excess` CONTRAST.** This campaign's two measured effect sizes — C's general valence **+0.025** and D2's **+0.01525** — are on the `A_|valence|` scale; they license H3's and H6's readings and say nothing about H2's. **An MDE divided by a LEVEL is not a power statement, and no threshold was ever named at which "wide margin" would stop being true.** The ratio itself has been wrong twice — a first draft claimed 3% (the understated figure's), a later one 5.3% (the one-sided z's) — and **it has now roughly doubled while the adjective in front of it never moved once**, which is how the withdrawal was found. **What survives is the number, its denominator, its alpha and its sidedness. What H2 can detect is stated; whether that is "enough" is not a question this registration has the effect size to answer, and it now says so instead of asserting it.**
...
**Each of these has a defensible alternative and a silent default. Q states which it takes and why, because an unstated estimator choice is a result the reader cannot price.**
    ...
    **U2 — CLUSTER WEIGHTING: UNWEIGHTED, AND THE ALTERNATIVE IS PRICED
    ...
    regardless of its cell count.
        ...
        **SE unweighted        0.001305    80% MDE  0.00436**
        **SE inverse-variance  0.001116    80% MDE  0.00373**
...
**"SIGNIFICANT" IN EVERY BRANCH BELOW MEANS `p < 0.0125`** — §Q3.1's FOUR-way split of alpha 0.05 over H1, H2, H4 and H5, **repeated here at the point of use.** H3 and H6 are estimated and appear in no branch. A reading rule that names its threshold only in another section is a reading rule whose first reader supplies the number, and the first draft of this registration named it in no section at all.
...
### §Q5.1 THE SUBSTITUTION ARMS — H1 x H2, on `tail_excess`
    ...
    ONE significant, the other null
        **NOT SUPPORTED as a general statement**, reported as the frame
        difference it is, with the significant arm named, **AND THE NULL
        ARM QUOTED AS A BOUND** — 0.00189 for H1, 0.00436 for H2
    BOTH null
        **NOT SUPPORTED, AND BOTH QUOTED AS BOUNDS, NEVER AS ABSENCES.**
        H1 excludes a paired site effect on `tail_excess` larger than
        **0.00189**; H2 excludes a cluster-level one larger than
        **0.00436**. **NEITHER BOUND MAY BE CALLED SMALL.** §Q4 records
        that this campaign has measured no effect size on the
        `tail_excess` scale — its two, C's +0.025 and D2's +0.01525, are
        both `A_|valence|` — so **there is nothing to call these bounds a
        fraction OF.** They may be reported, compared to each other, and
        left at that. The one comparison available is internal: H1's
        bound is **0.43 of H2's**, which is the pair frame's control
        showing up as arithmetic and is not an effect size either.
        **TWO DIGITS, BECAUSE THAT IS ALL THE PUBLISHED INPUTS SUPPORT.**
        This read `0.4326`, which is correct and is reachable by exactly
        ONE route — H1's SE rebuilt unrounded from `0.014765/sqrt(684)`,
        over H2's SE as published. **The two routes a reader actually has
        give two OTHER answers:** both SEs as published -> 0.4330, both
        MDEs as published -> 0.4335. **And H2's SE carries four
        significant figures, so its own rounding alone moves the ratio
        across [0.4324, 0.4328].** Three routes, three values, and the
        only figure all of them agree on is **0.43** — which is the whole
        of what this sentence claims. **A precision the document cannot
        let anyone verify buys nothing here and costs a future round**,
        in a clause already labelled as not an effect size.
        **BOTH BOUNDS ARE QUOTED AT THE REALIZED DISPERSION**, with the
        pre-registered figure printed beside them — §Q5.2's rule, which
        says ALWAYS and means it. **It is restated here because it is
        written there**, inside H4's section and argued entirely from
        H4's BORROWED prior, and a rule a reader meets only in another
        arm's branch is the defect §Q4's limit just had. H1's and H2's
        dispersions are **N's own, measured blind**, so the gap they close
        is pre-registered-vs-realized rather than borrowed-vs-realized —
        **narrower than H4's, and not zero:** the blind pass measured the
        frozen population, the producer measures what its run retains.
...
**THE NULL BRANCHES HERE CARRY BOUND LANGUAGE BECAUSE THEY DID NOT, AND THE ASYMMETRY RAN THE WRONG WAY.** §Q5.2 and §Q5.3 have required bound-not-absence language since they were written — **and those are H4 and H5, the two arms that HAVE a scale to bound against (L's 0.0789).** H1 and H2, which have **no** benchmark on their scale, stood at a bare `NOT SUPPORTED`. **So the arm where "not supported" is hardest to interpret was the arm where this registration did least to stop it being read as an absence.** §Q4's paragraph named that limit; a limit named in a power section and absent from the branch a reader executes is a limit that does not bind. **It binds here now.**
...
**THE BOUND IS RESTATED AT THE REALIZED DISPERSION, ALWAYS, AND THE TWO NUMBERS DO DIFFERENT JOBS.** H4's registered MDE is **0.00571**, sized on a **borrowed** prior — L's found-prose within-cluster SD of `departed`, 0.0789 — which is **7.2% of that SD**. §Q4 already requires the producer to report the **REALIZED** within-cluster SD of `departed` on N's own population.
...
### §Q5.3 THE PAIR-FRAME ARMS — H5 and H6, WHICH COMPLETE THE GRID

**Both are within-pair contrasts on the same unit as H1 and read the same way.**

    **H5 SIGNIFICANT, more mass departing at the MARKED member**
        **CONFIRMED — magnitude is elevated at the site relative to one
        word away from it.** With H4 this gives the magnitude row both
        cells and therefore its RATIO.
    **H6 IS NOT TESTED AND HAS NO BRANCH.** Its measured dispersion put
        its MDE at **1.16x** the only effect estimate available at the
        alpha it carries (1.42x at the 0.01 the demotion was decided
        on — **the figure never travels without its alpha**), so it reports
        a point estimate with an interval and **the word "confirmed" may
        not attach to it** — H3's disposition exactly. **The NORMS ROW
        THEREFORE HAS NO TESTED CELL IN EITHER COLUMN.**
    **H5 NULL**
        **NOT SUPPORTED, quoted as a BOUND at the realized dispersion**,
        never as an absence.
    **H5 SIGNIFICANT IN THE UNEXPECTED DIRECTION**
        **REPORTED AS A REVERSAL** — and see §Q7, because for these two
        arms the expected direction was known before the arm was written.
...
- **H5 AND H6 HAD A KNOWN EXPECTED DIRECTION BEFORE THEY WERE WRITTEN, AND THAT MAKES THEM REGISTERED REPLICATION RATHER THAN DISCOVERY.** The case for admitting each was a published sibling that already found the effect, and found it POSITIVE:

      **H5** — G, magnitude at sites in the pair frame: **d = 0.748, p 6.0e-05**
      **H6** — D2, valence extremity at the pair frame: **D = 0.015246, n 632, p 0.0076**

  **Q's arms are their own measurements on their own population** — 684 pairs against D2's 632, 33 clusters against G's 34, one instrument instead of two — **but nobody proposing H5 or H6 was blind to which way they were expected to come out.** A confirmed H6 is **NOT independent evidence of D2's sign**, and a confirmed H5 is not independent of G's. **This does not weaken either arm; it fixes what a confirmation means.** Same discipline as the known answers §Q6.1 withdrew: *a match is not independent confirmation of anything.*
- **THE TWO CELLS OF EVERY ROW ARE DIFFERENTLY-CONSTRUCTED DIFFERENCES, AND THE RATIO BETWEEN THEM IS LEGITIMATE ONLY WHEN THAT IS STATED.** The twin arm is a **WITHIN-PAIR** difference; the corpus arm is a **BETWEEN-PARTITION difference of cluster means.** Same measure, same units, **different KINDS of contrast.**

      **so "X is N times stronger at transgressive sites than in the corpus
      at large" is a ratio of two differently-constructed differences.**
      It is a real quantity and it is not a like-for-like comparison.

  **This caveat is now UNIFORM across all three rows, which it was not before H5.** Previously the magnitude row's twin cell lived in G as a standardised `d` while its corpus cell was a raw difference — **incommensurable, not merely differently constructed.** H5 puts the pair-frame magnitude in Q's own units, so every row now carries the same caveat and no row carries a worse one.
- **THE NORMS ROW HAS NO TESTED RATIO.** H6 tests the twin cell; the corpus cell is H3, **estimated and never tested**, because its MDE at the cluster unit (0.03693) exceeds both effects this campaign has measured. **A norms ratio may be reported as a point estimate with an interval and never as a verdict.** Substitution and magnitude have both cells tested; norms does not.
